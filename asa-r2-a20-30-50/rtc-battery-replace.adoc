...
summary: 更換 ASA A20 ， ASA A30 或 ASA A50 系統中的即時時鐘（ RTC ）電池（通常稱為幣式電池），以確保仰賴精確時間同步的服務和應用程式仍能正常運作。 
---
= 更換即時時鐘電池 - ASA A20 ， ASA A30 和 ASA A50
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
更換 ASA A20 ， ASA A30 或 ASA A50 系統中的即時時鐘（ RTC ）電池（通常稱為幣式電池），以確保仰賴精確時間同步的服務和應用程式仍能正常運作。

您可以更換控制器中的即時時鐘（ RTC ）電池，讓儲存系統的服務和應用程式能夠繼續運作，而這些服務和應用程式則需仰賴精確的時間同步。
...
. 在健全的控制器上，使用命令檢查日期和時間 `cluster date show`。
+

NOTE: 如果您的儲存系統停止在開機功能表，請在出現提示時選取選項 `Reboot node`並回應 _y_ ，然後按下 _Ctrl-C_ 以開機至 Loader 。

. 在功能受損的控制器上，在 Loader 提示下，檢查時間和日期： `cluster date show`
+
.. 如有必要，請修改日期： `set date _mm/dd/yyyy_`
.. 如有必要，請在 GMT 中設定時間： `set time hh:mm:ss`
.. 確認日期和時間。
