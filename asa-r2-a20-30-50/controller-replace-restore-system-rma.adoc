...
summary: 還原 NetApp 儲存加密組態（如有必要），安裝替換控制器的授權，驗證邏輯介面（生命週期）是否回報其主連接埠，執行叢集健全狀況檢查，登錄更換控制器的序號，然後將故障零件退回 NetApp ，以完成 ASA A20 ， ASA A30 或 ASA A50 控制器更換程序的最後步驟。 
---
...
[role="lead"]
還原 NetApp 儲存加密組態（如有必要），安裝替換控制器的授權，驗證邏輯介面（生命週期）是否回報其主連接埠，執行叢集健全狀況檢查，登錄更換控制器的序號，然後將故障零件退回 NetApp ，以完成 ASA A20 ， ASA A30 或 ASA A50 控制器更換程序的最後步驟。
