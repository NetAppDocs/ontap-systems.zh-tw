...
summary: 如果偵測到過多可修正或無法修正的記憶體錯誤，請更換ASA A20、 ASA A30 或ASA A50 儲存系統中的 DIMM。此類錯誤可能會阻止儲存系統啟動ONTAP。更換過程包括關閉受損的控制器、將其移除、更換 DIMM、重新安裝控制器，然後將故障部件傳回NetApp。 
---
= 更換 ASA A20、ASA A30 或 ASA A50 系統中的 DIMM 以修復記憶體容量錯誤
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如果偵測到過多可修正或無法修正的記憶體錯誤，請更換ASA A20、 ASA A30 或ASA A50 儲存系統中的 DIMM。此類錯誤可能會阻止儲存系統啟動ONTAP。更換過程包括關閉受損的控制器、將其移除、更換 DIMM、重新安裝控制器，然後將故障部件傳回NetApp。

當儲存系統遇到錯誤時、您必須更換控制器中的 DIMM 、例如根據健全狀況監視器警示或不可修正的 ECC 錯誤（可修正的錯誤修正碼）過多、通常是由於單一 DIMM 故障而導致、導致儲存系統無法開機 ONTAP 。

.開始之前
* 確保儲存系統中的所有其他元件都能正常運作；否則，請先聯絡 https://mysupport.netapp.com/site/global/dashboard["NetApp支援"]再繼續。」
