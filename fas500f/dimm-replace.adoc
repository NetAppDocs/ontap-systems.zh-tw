...
summary: 當系統登錄越來越多可修正的錯誤修正代碼時、您必須更換控制器模組中的 DIMM。否則會導致系統出現問題。 
---
...
[role="lead"]
當儲存系統出現諸如執行狀況監視器警報中顯示的過多 CECC（可修正錯誤修正碼）錯誤或無法修正的 ECC 錯誤等錯誤時，您必須更換控制器中的 DIMM 記憶體。這些錯誤通常是由單一 DIMM 記憶體條故障引起的，該故障會導致儲存系統無法開機 ONTAP。
