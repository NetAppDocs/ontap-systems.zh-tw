...
summary: 在關閉受損的控制器之前、您需要檢查系統是否已啟用NetApp Volume Encryption（NVE\）或NetApp Storage Encryption（NSE\）。如果是、您需要驗證組態。 
---
= 檢查內建加密金鑰- FAS500f
:allow-uri-read: 
:icons: font
...
+
** 如果命令輸出中顯示<lanne-dare>或<1Oanne-dare>、表示系統不支援NVE、請繼續關閉控制器。在關閉受損的控制器之前、您需要確認系統是否已啟用NetApp Volume Encryption（NVE）或NetApp Storage Encryption（NSE）。如果是、您需要驗證組態。
