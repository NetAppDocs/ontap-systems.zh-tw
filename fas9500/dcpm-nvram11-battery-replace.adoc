...
sidebar: sidebar 
keywords: FAS9500 
summary: 若要熱插拔包含 NVRAM11 電池的 destage 控制器電源模組 DCPM，您必須找到故障的 DCPM 模組、將其從機箱中移除，然後安裝替換的 DCPM 模組。 
---
= 更換 FAS9500 系統中的 DCPM 模組和 NVRAM11 電池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
在從機箱中移除故障的 DCPM 模組之前，您必須準備好備用 DCPM 模組。移除後必須在五分鐘內完成更換。DCPM 模組從機箱移除後，除了故障轉移到另一個控制器模組之外，擁有此 DCPM 模組的控制器模組將不再受到關機保護。
...
+
image::../media/drw_a900_remove_NV_battery.svg[更換 NV（DCPM）電池]
