...
sidebar: sidebar 
keywords: FAS9500 
summary: 本頁面提供安裝 FAS9500 系統的詳細逐步說明。 
---
= 請依照詳細說明安裝您的 FAS9500 系統
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
本頁提供安裝 FAS9500 系統的詳細逐步說明。如果您需要更詳細的安裝說明，請使用這些說明。
...
X6033A（112-00438）、3公尺
 a| 
image::../media/oie_cable_mini_sas_hd_to_mini_sas_hd.png[Mini SAS 連接器]
 a| 
儲存設備
...
將插槽 A4 和 B4 （e4b） 以及插槽 A8 和 B8 （e8b） 中的主機端 b 連接埠連接至主機交換器。
...
image::../media/icon_square_5_grey.png[圖說文字圖示 4.]
 a| 
** 將纜線固定在纜線管理臂上（未顯示）。
** 將電源線連接到電源供應單元 （PSU），並將電源供應單元連接到不同的電源（圖中未顯示）。PSU1 和 PSU3 為 A 面所有組件供電，而 PSU2 和 PSU4 為 B 面所有組件供電。
...
* 請務必參考圖示箭頭，確認電纜連接器拉片的正確方向。儲存設備模組的電纜拉片向上，而機櫃上的拉片向下。
...
.步驟
. 使用下列動畫或圖面，將控制器連接至三個（一個磁碟機機櫃的堆疊和兩個磁碟機機櫃的堆疊）DS224C 磁碟機機櫃。
+
.動畫電纜驅動器架
...
image::../media/oie_cable_mini_sas_hd_to_mini_sas_hd.png[Mini-SAS HD 電纜]
...
+
image::../media/drw_a900_power-on_IEOPS-941.svg[開啟系統電源]
