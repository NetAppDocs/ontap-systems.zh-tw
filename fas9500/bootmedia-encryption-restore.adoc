...
summary: 檢查環境變數後，必須完成已啟用 Onboard Key Manager （OKM）、NetApp Storage Encryption （NSE） 或 NetApp Volume Encryption （NVE） 的系統的特定步驟。 
---
= 手冊開機恢復程序完成後，還原 FAS9500 系統中開機媒體上的加密。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 FAS9500 系統中更換開機媒體後，請依照金鑰管理程式類型（例如 Onboard Key Manager （OKM））的步驟還原加密。
