...
---
= 更換 FAS9500 系統中的 NVRAM 模組或 NVRAM DIMM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
FAS9500 系統中的 NVRAM 模組由 NVRAM11 和 DIMM 組成。您可以更換故障的 NVRAM 模組或 NVRAM DIMM，以確保系統正常運作。

若要更換故障的 NVRAM 模組，必須將其從機箱中移除，將 DIMM 移至替換模組，然後將替換的 NVRAM 模組安裝到機箱中。

若要更換 NVRAM DIMM，必須將 NVRAM 模組從機箱中移除，更換模組中故障的 DIMM，然後重新安裝 NVRAM 模組。
...
開機替換控制器時，必須確認系統 ID 變更，然後驗證變更是否正確。
...
+
MetroCluster 更換設備後，組態需要幾分鐘才能恢復正常狀態。此時，每個控制器都會顯示已配置的狀態，DR 鏡像功能已啟用，模式為正常。 `metrocluster node show -fields node-systemid` 命令輸出會顯示舊的系統 ID，直到 MetroCluster 組態恢復正常狀態為止。
