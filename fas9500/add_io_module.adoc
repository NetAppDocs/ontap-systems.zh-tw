...
[role="lead"]
您可以為 FAS9500 系統新增 I/O 模組，方法是在已安裝所有元件的系統中以新的 NIC 或儲存介面卡取代舊的 NIC 或儲存介面卡，或將新的 NIC 或儲存介面卡新增至空的機箱插槽。
...
NOTE: 請務必在所有未使用的 I/O 插槽中安裝擋板，以防止可能出現的散熱問題。
...
. 如果您使用插槽 3 或 7 進行連網，請使用 `storage port modify -node __<node name>__ -port __<port name>__ -mode network`命令將插槽轉換為連網用途。
. 對控制器B重複這些步驟
