...
summary: 當儲存系統遇到諸如基於 Health Monitor 警示的過多 CECC 錯誤或無法修正的 ECC 錯誤等錯誤時，必須更換控制器中的 DIMM，這些錯誤通常是由單一 DIMM 故障引起的，導致儲存系統無法啟動 ONTAP。 
---
= 更換 FAS9500 系統中的 DIMM 以修復記憶體容量錯誤
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當儲存系統出現諸如基於 Health Monitor 警報的過多 CECC （Correctable Error Correction Codes）錯誤或不可糾正的 ECC 錯誤等錯誤時，您必須更換控制器中的 DIMM。這些錯誤通常是由單一 DIMM 故障所引起，導致儲存系統無法開機 ONTAP。
