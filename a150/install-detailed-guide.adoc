...
[role="lead"]
請按照這些詳細的逐步說明安裝您的 AFF A150 儲存系統，從準備站點到啟動 ONTAP 叢集。

如果您有 MetroCluster 組態、請使用 https://docs.netapp.com/us-en/ontap-metrocluster/index.html["本文檔MetroCluster"^]。

*警告：*如果您要安裝 ONTAP 9.13.1P8 - 9.13.1P11、ONTAP 9.14.1P1 - 9.14.1P7 或 ONTAP 9.15.1 - 9.15.1P2，且您的系統包含 10 個或更多內部固態硬碟，請採取額外步驟以準備系統進行安裝。請參閱問題  https://mysupport.netapp.com/site/bugs-online/product/ONTAP/JiraNgage/CONTAP-285173["CONTAP-285173 - ADP 不會在具有 10 個或更多內部磁碟機的 AFF A150 上留下備用根分割區"^]。
...
** 儲存系統的機架空間
** 2號十字螺絲起子
** 用於將系統連接到網路交換器和具有網路瀏覽器的筆記型電腦或主控台的額外連網線
** 一台配備 RJ-45 介面並可存取網路瀏覽器的筆記型電腦或主控台
...
您可以將系統安裝在四柱機架或 NetApp 系統機櫃中（視情況而定）。
...
--
乙太網路組態:: 使用 Cat 6 RJ45 電纜將 e0c 到 e0f 連接埠連接到主機網路，如下圖所示。
+
--
...
+
.. 開啟檔案總管。
.. 在左側窗格中選擇 *Network*，按一下滑鼠右鍵，然後選擇 *重新整理*。
