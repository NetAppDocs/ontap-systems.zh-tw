...
summary: 若要將系統還原為完整作業，您必須還原 NetApp Storage Encryption 組態、為新控制器安裝授權、並將故障零件退回 NetApp，如套件隨附的 RMA 說明所述。 
---
= 完成 AFF A150 系統的控制器更換工作
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
若要完成 AFF A150 系統的控制器更換，還原 NetApp 儲存加密組態（如有必要），確認邏輯介面 （LIF） 正在向其主連接埠報告，執行叢集資訊健檢，並將故障元件退回 NetApp。
...
在將 _replacement_ 節點恢復服務之前、您應該驗證 LIF 是否在其主連接埠上、如果啟用了 AutoSupport，則註冊 _replacement_ 節點的序號、檢查叢集的健全狀況、關閉任何維護時段、並重設自動恢復功能。
