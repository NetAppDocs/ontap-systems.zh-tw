...
summary: 當儲存系統遇到諸如健康監視器警報中顯示的過多 CECC 錯誤或無法糾正的 ECC 錯誤等錯誤時，必須更換控制器中的 DIMM，這些錯誤通常是由單一 DIMM 故障引起的，導致儲存系統無法啟動 ONTAP。 
---
= 更換 AFF A150 系統中的 DIMM 以修復記憶體容量錯誤
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當儲存系統遇到太多可修正（CECC）或不可修正的記憶體容量錯誤時，請更換 AFF A150 系統中的記憶體條 (DIMM)。此類錯誤可能導致儲存系統無法開機 ONTAP。
