...
summary: 當系統偵測到快閃記憶體壽命即將耗盡或偵測到的 NVDIMM 記憶體條發生故障時，請更換 AFF A400 控制器模組中的 NVDIMM 記憶體條。未更換故障的 NVDIMM 記憶體條可能會導致系統崩潰。 
---
= 更換 AFF A400 系統中的 NVDIMM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當您的 AFF A400 系統偵測到快閃記憶體壽命即將結束或 NVDIMM 狀況不良時，請更換 NVDIMM。若未更換，可能會導致系統發生緊急狀況。
