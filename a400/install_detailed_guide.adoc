...
NOTE: 如果看不到插卡上的連接埠標籤、請檢查插卡的安裝方向（PCIe連接器插槽位於A400和FAS8300/8700插槽左側）、然後在中依產品編號尋找插卡 https://["NetApp Hardware Universe"^] 顯示連接埠標籤的擋板圖形。您可以使用找到卡片零件編號 `sysconfig -a` 命令或系統壓縮清單上的。
