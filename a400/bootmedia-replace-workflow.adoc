...
summary: 透過查看更換需求、檢查加密狀態、關閉控制器、更換啟動媒體、啟動復原映像、復原加密以及驗證系統功能，開始更換AFF A400儲存系統中的啟動媒體。 
---
= 使用手冊開機恢復流程的 AFF A400 開機媒體更換工作流程
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
首先，請使用手動開機恢復流程更換 AFF A400 系統中的開機媒體。您必須從 NetApp 支援網站下載對應的 ONTAP 恢復映像，並將其複製到 USB 磁碟機，才能執行恢復並將系統還原至正常運作狀態。
