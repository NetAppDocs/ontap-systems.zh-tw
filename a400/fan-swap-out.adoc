...
[role="lead"]
熱交換 AFF A400 系統中的風扇模組，以維持適當的散熱。風扇模組必須在移除後兩分鐘內更換，以防止過熱。
...
+
使用 BMC 訊息或注意 LED 來識別要更換的風扇模組。BMC 將風扇模組對應為 Fan#_#，其中第一個數字是插槽，第二個數字是列（1= 頂部、2= 底部）；例如，插槽 1、第二列 = Fan1_2。
