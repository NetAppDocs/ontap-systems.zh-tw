...
[role="lead"]
請按照這些詳細的逐步說明安裝您的 AFF A400 儲存系統，從準備站點到啟動 ONTAP 叢集。
...
image:../media/oie_cable_fiber_lc_connector.png["LC 光纖連接器"]
 a| 
用於夾層卡的 16 Gb FC 或 25 GbE 纜線（視訂單而定）
...
NOTE: 如果卡片上的連接埠標籤不可見，請檢查卡片的安裝方向。在 A400 和 FAS8300/8700 中，PCIe 連接器插槽位於插槽的左側。然後，在 https://hwu.netapp.com["NetApp Hardware Universe"^]中透過產品編號尋找該卡，以取得顯示連接埠標籤的擋板圖示。您可以使用 `sysconfig -a`命令或在系統包裝清單上找到卡片的產品編號。
...
+
image::../media/drw_A400_TNSC-network-cabling.png[無交換器的雙節點網路纜線]
...
+
image::../media/drw_a400_switched_network_cabling.png[交換式叢集網路纜線]
...
+
image::../media/drw_a400_one_ns224_shelves.png[將控制器模組連接至單一磁碟機櫃]
...
+
image::../media/drw_a400_two_ns224_shelves.png[將兩個機櫃連接至控制器]
...
+
image::../media/drw_a400_three_ds224c_shelves.png[將三個機櫃連接至控制器]
...
+
.. 開啟檔案總管。
.. 在左側窗格中選擇 *Network*，然後按一下滑鼠右鍵並選擇 *重新整理*。
