...
summary: 當您的系統註冊越來越多的可修正錯誤修正碼（correctable error correction codes）或不可修正的 ECC 錯誤時，請更換 AFF A220 控制器模組中的 DIMM。若未更換故障的 DIMM，可能會導致系統出現緊急狀況（panic）。 
---
= 更換 AFF A220 系統中的 DIMM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當儲存系統遇到過多的可修正 ECC 錯誤或不可修正 ECC 錯誤時，請更換 AFF A220 系統中的 DIMM，這通常是由單一 DIMM 故障引起的。
