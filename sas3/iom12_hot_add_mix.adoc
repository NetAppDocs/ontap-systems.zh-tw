...
summary: 如果需要額外的儲存設備、您可以將IOM12磁碟櫃（含IOM12模組的SAS磁碟櫃）熱新增至IOM6磁碟櫃堆疊（含IOM6模組的SAS磁碟櫃）、表示您可以混合堆疊。 
---
= 將IOM12磁碟櫃熱新增至IOM6磁碟櫃堆疊
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
對於DS460C磁碟櫃、您也可以使用磁碟櫃隨附的四個可拆式握把。將握把往上推、直到卡入定位、即可安裝（機箱兩側各兩個）。將磁碟櫃滑到軌道上時、請使用指旋栓拆下握把。

+
如果您要移動滿載的DS460C磁碟櫃、建議您使用機械式起重設備或起重設備。

+

NOTE: 全負載的DS460C磁碟櫃重量約為247磅（112公斤）。

+
image::../media/drw_ds460c_handles.gif[DRW ds460c控點]

. 如果您要安裝DS460C磁碟櫃、請將元件安裝到機架磁碟櫃中；否則、請執行下一步。
+
如果您購買的磁碟櫃中、每個磁碟機插槽中都沒有磁碟機、則必須確保：

+
** 前四個插槽（0、3、6和9）會在每個藥櫃中佔用。
+
如此可確保磁碟櫃中的通風良好。

** 在具有30個磁碟機的磁碟櫃中、其餘10個磁碟機會平均分佈在每個磁碟櫃的插槽1和10中。
+
下圖顯示磁碟機如何在磁碟櫃內的每個磁碟機匣中編號0至11。插槽0、3、6、9及 在包含30個磁碟機的機櫃中、每個抽屜的插槽1和10必須包含磁碟機。
...
* 您可以先連接IOM A路徑上的SAS連接埠、以纜線連接IOM12磁碟櫃、然後視堆疊連線的情況、針對IOM B路徑重複佈線步驟。
+
...
* 您可以參考下圖、將IOM12磁碟櫃連接至IOM6磁碟櫃堆疊、並採用橋接式MetroCluster 附加功能的功能。 image:../media/hot_adding_iom12_shelves_to_iom6_stack_in_bridge_attached_config.png[""]
