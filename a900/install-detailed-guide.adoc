...
[role="lead"]
請按照這些詳細的逐步說明安裝您的 AFF A900 儲存系統，從準備站台到啟動 ONTAP 叢集。
...
+
NS224 機櫃預設機櫃 ID 為 00 和 01。如需變更機櫃 ID，您必須建立一個工具並將其插入按鈕所在的孔中。如需詳細指示，請參閱 link:../ns224/change-shelf-id.html["變更機櫃ID - NS224機櫃"]。
