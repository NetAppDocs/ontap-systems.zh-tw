...
summary: 在手動開機媒體恢復過程中，關閉 AFF A900 儲存系統中的控制器，以防止資料遺失並維持系統穩定性。 
---
= 在手動開機恢復過程中，關閉 AFF A900 系統中的控制器。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在手動開機媒體恢復過程中，關閉 AFF A900 儲存系統中的控制器，以防止資料遺失並維持系統穩定性。
...
.大多數組態
--
完成 NVE 或 NSE 任務後，需要完成控制器的關機。

如果您的儲存系統運作的是ONTAP 9.17.1 或更高版本，請使用自動啟動復原程序。如果您的系統運行的是早期版本的ONTAP，則必須使用手動啟動復原程序。
