...
summary: 繼續執行更換程序，重新連接儲存設備和網路組態。 
---
= 重新連接 AFF A900 系統中的系統線纜並重新分配磁碟機
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
重新連接儲存設備和網路組態，並還原 NetApp Storage Encryption，使您的 AFF A900 系統恢復全面運作。

重新啟用儲存設備和網路連線、以繼續執行更換程序。
...
NOTE: 系統ID和磁碟指派資訊位於NVRAM模組中、此模組與控制器模組分開、不受控制器模組更換影響。
...
. 如果_replaced_控制器處於維護模式（顯示"*>"提示）、請結束維護模式、然後進入載入程式提示：「halt（停止）」
...
+
更換後、需要幾分鐘時間才能恢復正常狀態、此時每個控制器都會顯示已設定的狀態、並啟用DR鏡射和正常模式。MetroCluster「MetroCluster 'scee-node-SystemID'」命令輸出會顯示舊的系統ID、直到MetroCluster 該組態恢復正常狀態為止。
