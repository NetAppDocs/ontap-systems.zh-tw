...
sidebar: sidebar 
keywords: AFF A900 
summary: 若要熱插拔包含 NVRAM11 電池的降級控制器電源模組，請找出故障的 DCPM 模組、將其從機箱中移除，然後安裝替換的 DCPM 模組。 
---
= 更換 AFF A900 系統中包含 NVRAM11 電池的 DCPM 模組
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當您的 AFF A900 系統中的降級控制器電源模組 （DCPM） 發生故障時，請進行熱交換。DCPM 包含 NVRAM11 電池，必須在移除後五分鐘內更換，以防止資料保護遺失。
...
+
image::../media/drw_a900_remove_NV_battery.svg[取出 NV 電池]
