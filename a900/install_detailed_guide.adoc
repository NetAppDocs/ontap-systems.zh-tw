...
. 從控制器記錄系統序號。
+
image:drw_ssn_label.svg["顯示號碼位置的系統序號範例"]
...
X66240A-5（112-00600）、5公尺
 a| 
image:oie_cable_sfp_gbe_copper.svg["寬度 =85px"]
 a| 
網路纜線
...
X66100-5（112-00544）、5公尺
 a| 
image:oie_cable100_gbe_qsfp28.svg["寬度 =85px"]
 a| 
乙太網路資料、叢集網路
...
X6033A（112-00438）、3公尺
 a| 
image:oie_cable_fiber_lc_connector.svg["寬度 =85px"]
 a| 
FC光纖網路
...
X6562-R6（112-00196）、5公尺
 a| 
image:oie_cable_rj45.svg["寬度 =85px"]
 a| 
管理網路與乙太網路資料
...
不適用
 a| 
image:oie_cable_micro_usb.svg["寬度 =85px"]
 a| 
...
不適用
 a| 
image:oie_cable_power.svg["寬度 =85px"]
 a| 
開啟系統電源
...
+
image:drw_9500_lifting_icon.svg["重量舉升注意標籤"]

. 連接纜線管理裝置（如圖所示）。
+
image:drw_9500_cable_management_arms.svg["安裝纜線管理 ARM"]
...
image:drw_a900_controller_in _chassis_ID_IEOPS-856.svg["典型叢集組態"]
...
image:oie_cable_pull_tab_up.svg["纜線拉片方向"]
...
+
image:drw_a900_tnsc_network_cabling_IEOPS-933.svg["無交換器的雙節點網路纜線"]
...
| 步驟 | 在每個控制器上執行 
 ...
 a| 
image:oie_legend_icon_1_lg.svg["寬度 =30px"]
 a| 
纜線叢集互連連接埠：
...
image:oie_cable100_gbe_qsfp28.svg["寬度 =85px"]
 ...
 a| 
image:oie_legend_icon_2_lp.svg["寬度 =30px"]
 a| 
纜線控制器管理（扳手）連接埠。
...
image:oie_cable_rj45.svg["寬度 =85px"]
 ...
 a| 
image:oie_legend_icon_3_o.svg["寬度 =30px"]
 a| 
纜線25 GbE網路交換器：
...
image:oie_cable100_gbe_qsfp28.svg["寬度 =85px"]
 ...
 a| 
image:oie_legend_icon_4_dr.svg["寬度 =30px"]
 a| 
纜線32 Gb FC連線：
...
image:drw_a900fas9500_power_source_icon_IEOPS-1142.svg["寬度 =200px"]
...
+
image:drw_a900_switched_network_cabling_IEOPS-934.svg["寬度 = 500px"]
...
image:oie_cable_pull_tab_down.svg["寬度 =200px"]
...
+
image:drw_a900_NS224_one shelf_cabling_IEOPS-937.svg["寬度 = 500px"]
...
| 步驟 | 在每個控制器上執行 
 ...
 a| 
image:oie_legend_icon_1_mb.svg["寬度 =30px"]
 a| 
** 將控制器A連接埠E2A連接至機櫃上NSM A的連接埠e0a。
** 將控制器A連接埠e10b連接至機櫃NSM B上的連接埠e0b。


image:oie_cable100_gbe_qsfp28.svg["寬度 =50px"]
...
100 GbE纜線
 ...
 a| 
image:oie_legend_icon_2_lo.svg["寬度 =30px"]
 a| 
...
+
image:drw_a900_NS224_line_art_two shelf_cabling_IEOPS-1147.svg["寬度 = 500px"]

+
image:drw_a900_NS224_two_shelf_cabling_IEOPS-938.svg["寬度 = 500px"]
...
+
image:drw_a900_oie_change_ns224_shelf_ID_IEOPS-836.svg["寬度 = 500px"]
...
+
image:drw_a900_power-on_IEOPS-941.svg["寬度 = 500px"]
...
+
image:dwr_laptop_to_switch_only.svg["寬度 = 500px"]

. 選取ONTAP 列出的功能表圖示以探索：
+
image:drw_autodiscovery_controler_select.svg["寬度 = 500px"]
...
+
image:drw_A900_cable_console_switch_controller_IEOPS-953.svg["寬度 = 500px"]
