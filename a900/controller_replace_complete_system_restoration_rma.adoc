...
summary: 若要恢復 AFF A900 系統的全部功能，請重新連接儲存設備，如有必要，恢復 NetApp Storage Encryption 配置，並為新控制器安裝授權。 
---
= 完成 AFF A900 系統的控制器更換
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
若要完成 AFF A900 系統的控制器更換，還原 NetApp Storage Encryption 組態（如有必要），確認邏輯介面 （LIF） 正在向其主連接埠報告，執行叢集資訊健檢，並將故障元件退回 NetApp。
