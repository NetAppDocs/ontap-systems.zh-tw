...
NOTE: 此程序僅適用於ONTAP HA配對中執行的系統。


CAUTION: 只有在更換NVRAM模組時才需要重新指派磁碟。
...
. 在提示符下輸入叢集的內建金鑰管理程式密碼：需要範例、請按這裡
. 輸入備份金鑰資料
+

NOTE: 完成通關密碼資訊後、節點會返回開機功能表。
...
. 在提示中輸入管理憑證資訊：需要範例於此處*
+

NOTE: 管理憑證資訊完成後、節點會返回開機功能表。
...
. 重設磁碟的MSID：
+
.. 如果您使用內建儲存加密、請依照中的程序進行 https://docs.netapp.com/us-en/ontap/encryption-at-rest/assign-authentication-keys-seds-onboard-task.html["指派資料驗證金鑰給FIPS磁碟機或SED（內建金鑰管理）"^]
.. 如果您使用的是外部Keymanager、請依照中的程序進行 https://docs.netapp.com/us-en/ontap/encryption-at-rest/assign-authentication-keys-seds-external-task.html["指派資料驗證金鑰給FIPS磁碟機或SED（外部金鑰管理）"^]
