...
---
= 依照詳細的逐步說明安裝 FAS8300 或 FAS8700 系統
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
這些詳細的逐步說明將引導您完成 FAS8300 或 FAS8700 系統的安裝，從機架安裝、佈線到初始設定。如果您需要比快速步驟更深入的安裝指導，請使用這些步驟。
...
NOTE: 如果卡片上的連接埠標籤不可見，請檢查卡片的安裝方向（在 A400 和 FAS8300/8700 中，PCIe 連接器插槽位於卡片插槽的左側），然後根據產品編號在 https://hwu.netapp.com["NetApp Hardware Universe"^]中尋找該卡，以取得顯示連接埠標籤的擋板圖示。您可以使用 `sysconfig -a`命令或在系統裝箱單上找到卡片的產品編號。
...
+
image::../media/drw_fas8300_TNSC_network_cabling.png[雙節點無交換器叢集佈線圖]
...
+
image::../media/drw_fas8300_switched_network_cabling.png[交換式叢集網路纜線圖]
...
+
image::../media/drw_fas8300_three-ds224c_shelves.png[三個 DS224C 機櫃佈線圖]
...
+
.. 開啟檔案總管。
.. 在左側窗格中選擇 *Network*，右鍵單擊 *Network*，然後選擇 *Refresh*。
.. 按兩下ONTAP 任一個「資訊」圖示、並接受畫面上顯示的任何憑證。
+

NOTE: 在本例中，XXXXX 代表目標節點的系統序號。
