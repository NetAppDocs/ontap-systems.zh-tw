...
summary: 當系統收到模組離線的單一 AutoSupport 訊息時、您必須更換控制器模組中的快取模組。 
---
= 更換 FAS8300 或 FAS8700 系統中的快取模組
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: VER2 控制器模組在 FAS8300 中只有一個快取模組插槽。FAS8700 沒有 VER2 控制器模組。移除插槽不會影響快取模組的功能。
...
NOTE: 插槽 6 僅適用於 FAS8300 VER2 控制器。

您的儲存系統必須符合特定條件、視您的情況而定：

* 它必須具備適用於您所安裝之快取模組的作業系統。
* 它必須支援快取容量。
* 雖然快取模組的內容已加密，但在更換模組之前，清除其內容是最佳實務做法。如需詳細資訊，請參閱 NetApp 支援網站上適用於您系統的 https://mysupport.netapp.com/info/web/ECMP1132988.html["波動性聲明"]。
+

NOTE: 您必須登入NetApp支援網站、才能顯示系統的_波動聲明_。

* 儲存系統中的所有其他元件必須正常運作。如果發生故障，請聯絡技術支援。
...
. 使用控制器模組上的 FRU 映射表找到故障的快取模組，並將其移除：
+
根據您的組態，控制器模組中可能包含零個、一個或兩個快取模組。請使用控制器模組內的 FRU 映射表來協助定位快取模組。
