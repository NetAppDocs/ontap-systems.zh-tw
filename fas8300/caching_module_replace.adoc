...
+
.. 清除快取模組上的資料：「System Controller flash-cache secure-erase run -node_node_name_-device-id _device_id_」
.. 驗證資料是否已從快取模組中清除：「System Controller flash-cache secure-erase show -node_node_name_」（系統控制器快閃快取安全清除show -node_node_name_）
+
輸出應顯示快取模組狀態為「已清除」。
...
https://netapp.hosted.panopto.com/Panopto/Pages/embed.aspx?id=d5f75ac5-b4f3-4b49-984b-aad9012f0c8e["更換快取模組"]
...
+
視您的組態而定、控制器模組中可能有零、一個或兩個快取模組。故障快取模組的LED會亮起。
...
.步驟
. 如果要維修的控制器未出現在載入器提示字元、請重新啟動控制器：「ystem nodeat -node_node_name_」（系統節點停止節點節點_node_name_）
+
