...
summary: 開始更換FAS8300或FAS8700儲存系統中的啟動介質，方法是檢視更換需求、關閉控制器、更換啟動介質、恢復啟動介質上的映像並驗證系統功能。 
---
= 使用自動化開機恢復流程的 FAS8300 和 FAS8700 開機媒體更換工作流程
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
首先，使用自動開機恢復流程更換 FAS8300 或 FAS8700 儲存系統中的開機媒體。系統會自動辨識並選擇對應的開機功能表選項，然後使用合作夥伴節點上的開機映像在取代的開機媒體上重新安裝 ONTAP。若要完成更換，請審查相關要求、關閉控制器、更換開機媒體、允許系統還原映像，並驗證系統功能。
