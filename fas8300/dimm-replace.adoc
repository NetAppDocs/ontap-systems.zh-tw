...
summary: 當儲存系統遇到諸如過多的可修正 ECC 記憶體容量錯誤或不可修正 ECC 錯誤等錯誤時，必須更換控制器中的 DIMM，這些錯誤通常是由單一 DIMM 故障引起的，導致儲存系統無法開機 ONTAP。 
---
= 更換 FAS8300 或 FAS8700 系統中的 DIMM 以修復記憶體容量錯誤
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當儲存系統出現諸如 Health Monitor 警報提示的過多可糾正 ECC 記憶體容量錯誤或不可糾正 ECC 錯誤等情況時，必須更換控制器中的 DIMM。這些錯誤通常是由單一 DIMM 故障引起的，導致儲存系統無法開機 ONTAP。

系統中的所有其他組件必須正常運作。如果某個組件無法正常運作，您必須聯絡技術支援。
...
系統中 DIMM 的數量和位置取決於您的系統型號。如需更多資訊，請參閱風道上的 FRU 圖。

* 如果您使用FAS8300系統、則系統DIMM位於插槽2、4、13和15中。
* 如果您有FAS8700系統、則系統DIMM位於插槽2、4、7、9、13、 15、18和20。
* NVDIMM位於插槽11。
...
NOTE: 您將在此程序稍後將其餘纜線連接至控制器模組。

. 完成控制器模組的安裝：
+
.. 使用鎖定閂將控制器模組牢固地推入機箱，直到鎖定閂開始上升。
+
