...
summary: 當儲存系統遇到諸如過多的可修正錯誤修正代碼或不可修正錯誤等錯誤時,必須更換控制器中的 DIMM,這通常是由單一 DIMM 故障引起的,導致儲存系統無法啟動 ONTAP。 
---
...
[role="lead"]
當儲存系統出現諸如 Health Monitor 警報中顯示的過多 CECC （可糾正錯誤校正碼）錯誤或無法糾正的 ECC 錯誤等錯誤時，您必須更換控制器中的 DIMM。這些錯誤通常是由單一 DIMM 故障引起的，該故障會導致儲存系統無法開機 ONTAP。
