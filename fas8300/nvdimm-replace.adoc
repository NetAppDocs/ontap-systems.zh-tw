...
summary: 當系統偵測到快閃記憶體壽命即將結束或偵測到已識別的 NVDIMM 記憶體條狀況不佳時，必須更換控制器模組中的 NVDIMM 記憶體條。 
---
= 更換 FAS8300 或 FAS8700 系統中的 NVDIMM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
系統中的所有其他組件必須正常運作。如果它們發生故障，您必須聯絡技術支援。
...
要更換 NVDIMM，您必須使用插槽 1 立管頂部的 FRU 映射表在控制器模組中找到它。

* 當您停止系統時、NVDIMM LED會在減少內容時閃爍。完成轉出後、LED會關閉。
* 儘管 NVDIMM 的內容已加密，但最佳實務做法是在更換 NVDIMM 之前擦除其內容。如需更多資訊，請參閱 NetApp 支援網站上的 https://mysupport.netapp.com/info/web/ECMP1132988.html["波動性聲明"]。
+
...
. 將NVDIMM正面插入插槽。
+
NVDIMM 與插槽連接緊密，但應該可以輕鬆插入。如果無法插入，請重新對準 NVDIMM 與插槽的位置，然後重新插入。
...
. 將控制器恢復正常運作：「儲存容錯移轉還原-ofnode_disapped_node_name_」
