...
summary: 若要完成 AFF C250 系統的控制器更換以使其完全運作，您必須還原 NetApp Storage Encryption 組態、安裝新控制器的授權、並將故障零件退回 NetApp。 
---
= 完成 AFF C250 儲存系統的控制器更換
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
要完成 AFF C250 系統的控制器更換並使其完全運行，您必須恢復 NetApp 儲存加密配置（如有必要），並為新控制器安裝授權，然後按照套件附帶的 RMA 說明將故障部件退回 NetApp。
