...
[role="lead"]
完成硬體更換並將 AFF C250 系統開機至維護模式後、您需要驗證更換控制器的低階系統組態、並視需要重新設定系統設定。
...
.接下來呢？
恢復並驗證系統組態後，您需要link:controller-replace-recable-reassign-disks.html["重新連接系統線纜並重新指派磁碟"]。
