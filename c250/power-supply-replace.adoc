...
. 旋轉 CAM 把手、以便在按下鎖定彈片的同時、將 PSU 從控制器模組中拉出。
+
...
|===
. 用雙手支撐 PSU 的邊緣、並將其與控制器模組的開口對齊、然後將 PSU 輕輕推入控制器模組、直到鎖定彈片卡入定位。
+
...
.. 從 PSU 拔下 D-sub DC 電源線接頭、並將其放在一邊。
...
.. 將握把向上旋轉至水平位置、然後加以抓住。
.. 用您的姆指按下藍色彈片以釋放鎖定機制。
.. 將 PSU 從控制器模組中拉出、同時用另一隻手支撐其重量。
+

NOTE: PSU很短。從控制器模組移除時、請務必用雙手支撐、以免其從控制器模組中迴轉而造成傷害。
...
.. 鎖緊兩顆指旋螺絲、將 D-sub 直流電源纜線連接器固定至 PSU 。
