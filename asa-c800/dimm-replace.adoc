...
summary: 如果在 ASA C800 系統中偵測到過多可修正或不可修正的記憶體錯誤，請更換記憶體條（DIMM）。此類錯誤可能會導致儲存系統無法啟動 ONTAP。 
---
= 更換 ASA C800 系統中的 DIMM 以修復記憶體容量錯誤
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如果偵測到過多的可修正或不可修正的記憶體錯誤，請更換ASA C800系統中的 DIMM。此類錯誤可能會阻止儲存系統啟動ONTAP。更換過程包括關閉受損的控制器、將其移除、更換 DIMM、重新安裝控制器，然後將故障部件傳回NetApp。
