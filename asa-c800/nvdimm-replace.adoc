...
summary: 當系統偵測到快閃記憶體壽命即將耗盡或偵測到已識別的 NVDIMM 不健全時、您必須更換控制器模組中的 NVDIMM。否則會導致系統出現問題。 
---
= 更換 ASA C800 系統中的 NVDIMM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當您的ASA C800系統偵測到快閃記憶體壽命即將結束或偵測到已識別的 NVDIMM 整體上不健康時，請更換系統中的 NVDIMM；否則會導致系統崩潰。

.開始之前
* 請確保您已收到從NetApp收到的替換 NVDIMM，並且該 NVDIMM 與您的ASA C800系統相容。
