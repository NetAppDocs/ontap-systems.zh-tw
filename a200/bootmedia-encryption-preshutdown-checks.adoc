...
summary: 在關閉故障控制器並檢查板載加密金鑰的狀態之前，必須檢查故障控制器的狀態，停用自動返還，並檢查系統正在執行的ONTAP版本。AFF A200系統僅支援手動啟動媒體復原程式。 
---
= 檢查 AFF A200 系統中開機媒體的加密支援情況
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
為確保儲存系統的資料安全，您需要驗證啟動媒體上的加密金鑰支援情況和狀態。檢查您的ONTAP版本是否支援NetApp磁碟區加密 (NVE)，並在關閉控制器之前檢查金鑰管理員是否處於活動狀態。AFF A200系統僅支援手動啟動媒體復原程式。不支援自動啟動媒體恢復。
