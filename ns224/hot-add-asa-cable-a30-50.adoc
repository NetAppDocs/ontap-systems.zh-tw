...
summary: 將 NS224 機架連接到 ASA A30 或 ASA A50 系統，使每個機架與 HA 配對中的每個控制器都有兩個連接。 
---
= 將 NS224 機架連接線連接到您的 ASA A30 或 ASA A50 系統
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
將 NS224 機架連接到 ASA A30 或 ASA A50 系統，使每個機架與 HA 配對中的每個控制器都有兩個連接。

當需要額外的儲存設備（內部機櫃之外）時、您可以將最多兩個 NS224 機櫃熱新增至 ASA A30 或 ASA A50 HA 配對。
