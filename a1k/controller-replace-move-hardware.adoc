...
summary: '當硬體故障需要更換AFF A1K系統中的控制器時，請更換控制器。更換過程包括從機殼前方移除控制器，移除電源單元 (PSU)、NVRAM12、I/O 模組和空白板以及系統管理模組，然後安裝替換機箱，並重新安裝機箱組件。' 
---
= 更換控制器 - AFF A1K
:allow-uri-read: 
:icons: font
...
[role="lead"]
當硬體故障需要更換AFF A1K系統的控制器時。更換過程包括移除受損的控制器、將組件移至更換的控制器、安裝更換的控制器以及重新啟動它。
...
.接下來呢？
更換損壞的 AFF A1K 控制器之後，您需要link:controller-replace-system-config-restore-and-verify.html["還原系統組態"]。
