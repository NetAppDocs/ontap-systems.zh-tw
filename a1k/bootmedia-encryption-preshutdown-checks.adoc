...
summary: 為了確保 AFF A1K 儲存系統上的資料安全性，您需要驗證開機媒體上的加密金鑰支援和狀態。請檢查您的 ONTAP 版本是否支援 NetApp Volume Encryption ，並在關閉控制器之前，檢查金鑰管理程式是否為作用中。 
---
= 檢查手動啟動媒體恢復的加密支援 - AFF A1K
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
為了確保 AFF A1K 儲存系統上的資料安全性，您需要驗證開機媒體上的加密金鑰支援和狀態。檢查您的 ONTAP 版本是否支援 NetApp Volume Encryption （ NVE ），以及在關閉控制器之前，檢查金鑰管理程式是否為作用中。
