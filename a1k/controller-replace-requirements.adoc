...
summary: 在更換 AFF A1K 系統中的控制器之前，請確認您符合成功更換的必要條件。這包括確認系統中的所有其他元件均運作正常、確認您擁有正確的替換控制器，以及將控制器的主控台輸出儲存至文字日誌檔。 
---
...
[role="lead"]
在更換 AFF A1K 系統中的控制器之前，請確認您符合成功更換的必要條件。這包括確認系統中的所有其他元件均運作正常、確認您擁有正確的替換控制器，以及將控制器的主控台輸出儲存至文字日誌檔。
...
.接下來呢？
在您檢閱更換 AFF A1K 控制器的需求之後，您需要link:controller-replace-shutdown.html["關閉受損的控制器"]。
