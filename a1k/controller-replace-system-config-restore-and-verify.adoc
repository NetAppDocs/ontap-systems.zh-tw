...
summary: 確認控制器的 HA 組態在 AFF A1K 儲存系統中為作用中且運作正常，並確認系統的介面卡列出磁碟的所有路徑。 
---
= 還原並驗證 AFF A1K 系統中的控制器組態
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
確認控制器的 HA 組態在 AFF A1K 儲存系統中為作用中且運作正常，並確認系統的介面卡列出磁碟的所有路徑。
...
.接下來呢？
在您還原並驗證 AFF A1K 系統的系統組態之後link:controller-replace-recable-reassign-disks.html["將控制器交回"]，您必須執行。
