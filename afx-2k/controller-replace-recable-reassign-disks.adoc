...
summary: 將儲存資源的控制權交還給替換控制器，以便您的 AFX 2K 儲存系統能夠恢復正常運作。 
---
= 請將控制器放回您的 AFX 2K 儲存系統中，以恢復正常運作。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
將儲存資源的控制權交還給替換控制器，以便您的 AFX 2K 儲存系統能夠恢復正常運作。交還控制權的步驟取決於您系統使用的加密類型：無加密或 Onboard Key Manager （OKM） 加密。
