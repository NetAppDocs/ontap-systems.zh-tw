...
summary: 如果偵測到 AFX 2K 儲存系統中存在過多可修正或不可修正的記憶體錯誤，請更換其中的 DIMM 記憶體條。此類錯誤可能導致儲存系統無法啟動 ONTAP。更換流程包括：關閉故障控制器，將其拆下，更換 DIMM 記憶體條，重新安裝控制器，然後將故障零件退回 NetApp。 
---
= 更換 AFX 2K 系統中的 DIMM 記憶體條以修復記憶體容量錯誤
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如果偵測到 AFX 2K 儲存系統中存在過多可修正或不可修正的記憶體錯誤，請更換其中的 DIMM 記憶體條。此類錯誤可能導致儲存系統無法啟動 ONTAP。更換流程包括：關閉故障控制器，將其拆下，更換 DIMM 記憶體條，重新安裝控制器，然後將故障零件退回 NetApp。
