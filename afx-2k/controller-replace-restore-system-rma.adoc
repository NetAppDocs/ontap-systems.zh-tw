...
summary: 驗證邏輯介面是否向其主連接埠報告，執行叢集資訊健檢，然後將故障零件退回 NetApp，以完成 AFX 2K 控制器更換程序的最後一步。 
---
= 完成 AFX 2K 儲存系統的控制器更換
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
驗證邏輯介面 （LIF） 是否向其主連接埠報告，執行叢集健康檢查，然後將故障零件退回 NetApp 以完成 AFX 2K 控制器更換程序的最後一步。
