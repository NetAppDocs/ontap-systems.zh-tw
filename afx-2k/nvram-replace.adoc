...
summary: 當您的 AFX 2K 儲存系統中的非揮發性記憶體故障或需要升級時，請更換 NVRAM。 
---
= 更換 AFX 2K 儲存系統中的 NVRAM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當您的 AFX 2K 儲存系統中的非揮發性記憶體故障或需要升級時，請更換 NVRAM。更換過程包括關閉故障控制器、更換 NVRAM 模組或 NVRAM DIMM，並將故障零件退回 NetApp。
...
+
image::../media/drw_afx_2k_nvram_remove_replace_ieops-2881.svg[取下 NVRAM12 模組和 DIMM]
...
. 從機箱中卸下目標 NVRAM 模組。
+
image::../media/drw_afx_2k_nvram_remove_replace_ieops-2881.svg[卸下 NVRAM 12 模組和 DIMM]
...
NOTE: 覆蓋系統 ID 僅在更換 NVRAM 模組時需要，而更換 NVRAM DIMM 時則不需要。
...
. 退回控制器：
+
.. 從健康的控制器，歸還被替換控制器的儲存設備：`_storage failover giveback -ofnode impaired_node_name_`
+
控制器會恢復其儲存設備並完成開機。
