...
summary: 當您的 AFX 2K 儲存系統出現硬體故障需要更換控制器時，請進行更換。更換過程包括：移除受損控制器、將組件移至替換控制器、安裝替換控制器，然後重新開機。 
---
= 更換控制器 - AFX 2K
:allow-uri-read: 
:icons: font
...
[role="lead"]
當您的 AFX 2K 儲存系統發生硬體故障時，請更換控制器。更換過程包括：卸下受損控制器、將組件移至替換控制器、安裝替換控制器，然後重新開機。
...
. 將DIMM正面插入插槽。
+
DIMM 緊緊地插入插槽，但您應該能夠輕鬆插入它。如果不是，請將 DIMM 與插槽重新對齊並重新插入。
...
.接下來呢？
更換損壞的 AFX 2K 控制器後，link:controller-replace-system-config-restore-and-verify.html["恢復系統設定"]。
