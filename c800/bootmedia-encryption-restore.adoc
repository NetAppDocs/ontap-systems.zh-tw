...
summary: 在AFF C800系統中，恢復替換啟動媒體上的加密，以確保資料持續受到保護。替換過程包括驗證金鑰可用性、重新套用加密設定以及確認對資料的安全存取。 
---
= 在手動開機恢復程序之後，還原 AFF C800 系統中開機媒體上的加密。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在AFF C800系統中，恢復替換啟動媒體上的加密，以確保資料持續受到保護。替換過程包括驗證金鑰可用性、重新套用加密設定以及確認對資料的安全存取。
