...
summary: 若要將 FAS2600 系統還原至完整運作狀態，您必須還原 NetApp Storage Encryption 組態、為新控制器安裝授權，並將故障零件退回 NetApp。 
---
= 完成 FAS2600 系統的控制器更換
:allow-uri-read: 
:icons: font
...
[role="lead"]
若要將 FAS2600 系統還原至完整運作狀態，您必須還原 NetApp 儲存加密組態（如有必要）、安裝新控制器的授權，並依照套件隨附的 RMA 說明，將故障零件退回 NetApp。
