...
summary: 當儲存系統根據 Health Monitor 警報遇到過多的可修正 ECC 錯誤，或由於單一 DIMM 故障導致系統無法開機 ONTAP 而出現不可修正的 ECC 錯誤時，請更換 FAS2600 系統中的 DIMM。 
---
= 更換 FAS2600 系統中的 DIMM 以修復記憶體容量錯誤
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當儲存系統根據執行狀況監視器警報遇到過多的 CECC （可修正錯誤修正代碼）錯誤，或遇到無法修正的 ECC 錯誤時，請更換 FAS2600 系統中的 DIMM，這通常是由單一 DIMM 故障引起的，導致儲存系統無法開機 ONTAP。

如果其他任何組件無法正常運作，請聯絡技術支援。

您必須使用從供應商處收到的替換FRU元件來更換故障元件。

link:https://youtu.be/ywwsMltvuXg["FAS2600 DIMM更換影片AFF"^]
