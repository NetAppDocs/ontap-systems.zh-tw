...
summary: 在 FAS2600 系統的替換開機媒體上還原加密組態。此程序包括對啟用了 Onboard Key Manager、NetApp Storage Encryption 或 NetApp Volume Encryption 的系統完成更換後步驟，以確保安全的資料存取和系統的正常運作。 
---
= 在 FAS2600 系統中恢復開機媒體上的加密
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 FAS2600 系統的替換啟動媒體上還原加密配置。此流程包括完成已啟用板載金鑰管理器 (OKM)、 NetApp儲存加密 (NSE) 或NetApp磁碟區加密 (NVE) 的系統更換後的步驟，以確保安全的資料存取和系統的正常運作。
