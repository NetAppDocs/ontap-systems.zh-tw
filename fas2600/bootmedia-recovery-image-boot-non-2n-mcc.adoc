...
summary: 在 FAS2600 系統上，從 USB 磁碟機啟動ONTAP復原映像，以復原啟動媒體。FAS2600 系統僅支援手動啟動媒體復原程序。不支援自動啟動媒體恢復。 
---
= 開機恢復映像- FAS2600
:allow-uri-read: 
:icons: font
...
[role="lead"]
在 FAS2600 系統上，從 USB 磁碟機啟動ONTAP復原映像，以復原啟動媒體。此過程包括從 USB 隨身碟啟動、還原檔案系統、驗證環境變數以及在更換啟動媒體後使控制器恢復正常運作。
