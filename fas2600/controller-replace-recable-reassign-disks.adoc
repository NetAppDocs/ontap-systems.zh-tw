...
summary: 重新連接儲存設備、確認磁碟重新分配，並完成剩餘任務，以交還控制器並將 FAS2600 系統還原至完整運作狀態。 
---
= 將 FAS2600 系統中的控制器交還，以恢復正常運作。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
重新連接儲存設備、確認磁碟重新分配，並完成剩餘任務，以交還控制器並將 FAS2600 系統還原至完整運作狀態。
...
. 在_replace_控制器的載入器提示中、啟動控制器、如果系統ID不相符、系統提示您置換系統ID、請輸入「y」：「boot_ONTAP」
