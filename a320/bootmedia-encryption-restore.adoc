...
summary: 檢查完環境變數後，您必須完成已啟用板載金鑰管理器、 NetApp儲存加密或NetApp磁碟區加密的系統的特定步驟。AFF A320系統僅支援手動啟動媒體復原程式。不支援自動啟動媒體恢復。 
---
= 在 AFF A320 系統中恢復開機媒體上的加密
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
恢復替換啟動媒體上的加密。AFF A320系統僅支援手動啟動媒體復原程式。不支援自動啟動媒體恢復。
