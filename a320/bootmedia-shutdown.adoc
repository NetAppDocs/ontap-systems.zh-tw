...
summary: 完成 NVE 或 NSE 任務後，需要完成受損節點的關閉。AFF A320系統僅支援手動啟動媒體復原程式。不支援自動啟動媒體恢復。 
---
= 在手動開機恢復過程中，關閉 AFF A320 系統中發生故障的控制器。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
完成 NVE 或 NSE 任務後，需要完成受損節點的關閉。使用適合您配置的相應程式關閉或接管故障控制器。AFF A320系統僅支援手動啟動媒體復原程式。不支援自動啟動媒體恢復。
