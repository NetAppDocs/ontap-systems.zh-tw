...
---
= 為您的 AFF A70 或 AFF A90 硬體連接網路和儲存設備。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
以下佈線步驟顯示如何將控制器連接到儲存設備機櫃。請選擇與您的設定相符的佈線方案之一。

如需儲存系統支援的最大機櫃數量以及所有纜線選項link:https://hwu.netapp.com["NetApp Hardware Universe"^]，請參閱。

.關於這項工作
AFF A70 和 AFF A90 儲存系統支援配備 NSM100 或 NSM100B 模組的 NS224 機架。這兩個模組的主要差異在於：
...
====
.接下來呢？
完成 AFF A70 或 AFF A90 系統的硬體佈線後，您link:install-power-hardware.html["開啟 AFF A70 或 AFF A90 儲存系統的電源"]。
