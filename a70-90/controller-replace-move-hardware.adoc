...
.接下來呢？
更換損壞的AFF A70或AFF A90控制器後，您需要：link:controller-replace-system-config-restore-and-verify.html["還原系統組態"] 。
