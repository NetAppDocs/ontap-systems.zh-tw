...
summary: 更換 AFF A70 或 AFF A90 系統中的即時時鐘電池（通常稱為紐扣電池），以確保依賴精確時間同步的服務和應用程式能夠繼續運作。 
---
= 更換 AFF A70 或 AFF A90 系統中的即時時鐘電池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
請在 AFF A70 或 AFF A90 系統中更換即時時鐘（ RTC ）電池（通常稱為幣式電池），以確保仰賴精確時間同步的服務和應用程式仍能正常運作。

.開始之前
* 瞭解您可以將此程序用於系統支援的所有 ONTAP 版本。
* 請確保系統中所有其他組件都運作正常。如果發生故障，請聯絡技術支援。
