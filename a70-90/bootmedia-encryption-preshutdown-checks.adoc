...
summary: 為確保 AFF A70 或 AFF A90 儲存系統的資料安全，您需要驗證啟動媒體上的加密金鑰支援情況和狀態。請檢查您的 ONTAP 版本是否支援 NetApp Volume Encryption，並在關閉控制器之前檢查金鑰管理程式是否處於作用中狀態。 
---
= 使用手冊開機恢復流程檢查 AFF A70 或 AFF A90 系統中開機媒體的加密支援
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
為確保 AFF A70 或 AFF A90 儲存系統的資料安全，您需要驗證啟動媒體上的加密金鑰支援情況和狀態。檢查您的 ONTAP 版本是否支援 NetApp 磁碟區加密 （NVE）。在關閉控制器之前，請檢查金鑰管理程式是否處於作用中狀態。
