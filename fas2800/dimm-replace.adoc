...
summary: 當 FAS2820 系統控制器出現諸如 Health Monitor 警報中顯示的過多可糾正 ECC 記憶體容量錯誤或不可糾正 ECC 錯誤（通常是由單一 DIMM 故障引起，導致儲存系統無法開機 ONTAP）時，必須更換控制器中的 DIMM。 
---
...
[role="lead"]
當 FAS2820 系統控制器出現諸如 Health Monitor 警報中顯示的過多可糾正 ECC 記憶體容量錯誤或不可糾正 ECC 錯誤（通常是由單一 DIMM 故障引起，導致儲存系統無法開機 ONTAP）時，必須更換控制器中的 DIMM。
