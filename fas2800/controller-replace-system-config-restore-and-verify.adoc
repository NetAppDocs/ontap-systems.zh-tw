...
summary: 完成硬體更換並在 FAS2820 系統中開機更換控制器後，請驗證更換控制器的底層系統組態，並視需要重新設定系統設定。 
---
= 還原並驗證 FAS2820 系統中的控制器組態
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
完成硬體更換並在 FAS2820 系統中開機更換控制器後，請驗證更換控制器的底層系統組態，並視需要重新設定系統設定。
...
. 如果控制器的顯示系統狀態與您的系統組態不符、請設定更換控制器模組的 HA 狀態： `ha-config modify controller _HA-state_`
+
HA狀態的值可以是下列其中一項：
