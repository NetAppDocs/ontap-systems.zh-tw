...
summary: 要更換 FAS2820 系統中的 NVMEM 電池，必須移除控制器模組、開啟它、更換電池，然後關閉並重新安裝控制器模組。 
---
= 更換 FAS2820 系統中的 NVMEM 電池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
要更換 FAS2820 系統中的 NVMEM 電池，必須移除控制器模組、開啟它、更換電池，然後關閉並重新安裝控制器模組。

如果系統中的任何其他組件無法正常運作，請聯絡 https://mysupport.netapp.com/site/global/dashboard["NetApp支援"]。
...
NOTE: 如果系統處於「等待歸還」狀態，或系統未正確接管或停止（存在未提交資料），則在控制器斷電時，面板上的綠色 NV LED 指示燈會開始閃爍。如果故障控制器模組未能被合作夥伴控制器模組成功接管，請聯繫 https://mysupport.netapp.com/site/global/dashboard["NetApp支援"]

+
** 如果從機箱中卸下控制器模組時綠色的 NV 狀態 LED 開始閃爍：
+
*** 確認控制器已由合作夥伴控制器模組順利接管，或故障控制器顯示 _waiting for giveback_。如果是，您可以忽略閃爍的 LED 指示燈，並繼續從機箱移除故障控制器。


** 如果綠色 NV LED 熄滅，則可以完成從機箱中移除受損控制器。
