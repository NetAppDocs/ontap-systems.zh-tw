...
summary: 在 FAS2820 系統中的替換開機媒體上還原 ONTAP 映像。此程序包括從 LOADER 提示字元啟動開機恢復程序，並監控恢復程序直至完成且節點重新啟動。 
---
= 使用自動開機恢復流程在 FAS2820 系統上啟動恢復映像
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 FAS2820 系統中安裝新的開機媒體裝置後，您可以啟動自動開機媒體還原程序，以從合作夥伴節點還原組態。在恢復程序期間，系統會檢查是否已啟用加密，並確認目前使用的金鑰加密類型。如果已啟用金鑰加密，系統將引導您完成對應的還原步驟。
