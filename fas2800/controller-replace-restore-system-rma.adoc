...
summary: 依照套件隨附的 RMA 說明中所述，恢復 NetApp Storage Encryption 或 Volume Encryption 組態、安裝替換控制器的授權、並將故障零件退回 NetApp，即可使系統恢復完全運作。 
---
= 完整系統還原： FAS2820
:allow-uri-read: 
:icons: font
...
[role="lead"]
還原 NetApp 儲存加密或磁碟區加密組態（如有必要）、安裝更換控制器的授權、並將故障零件退回 NetApp 、將系統還原至完整運作、如套件隨附的 RMA 指示所述。
