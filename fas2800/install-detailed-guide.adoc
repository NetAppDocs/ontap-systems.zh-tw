...
summary: 本程序提供安裝 FAS2820 儲存系統的詳細逐步說明。 
---
= 請依照詳細說明安裝您的 FAS2820 系統
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
本流程提供詳細的逐步說明，指導您如何準備、安裝、佈線和啟動 FAS2820 儲存系統。如果您偏好詳盡的說明而非簡短的快速指南，請使用這些步驟。
...
NOTE: 如果您的系統沒有外部磁碟櫃，請參閱 link:../sas3/install-cabling-worksheets-examples-fas2600.html#fas2820-platform-in-a-multipath-ha-configuration-with-no-external-shelves["在無外部儲存櫃的多路徑 HA 組態中 FAS2820 的佈線說明"]。
...
請使用選項 1（如果已啟用網路探索）或選項 2（如果未啟用網路探索）完成儲存系統的設定和組態。
...
.選項1：如果已啟用網路探索
--
如果您的筆記型電腦啟用了網路探索，請使用自動叢集探索完成儲存系統設定和組態。

.步驟
. 開啟機櫃電源，並按照此步驟開始時的動畫設定機櫃 ID。
. 開啟控制器電源
+
...
. 建立帳戶或登入您的帳戶。
+
.. 選擇 https://mysupport.netapp.com["mysupport.netapp.com"]
.. 如果您需要建立帳戶或登入帳戶，請選擇「建立帳戶」。
