...
summary: 驗證邏輯介面是否正在回報其主連接埠，執行叢集健全狀況檢查，並將故障零件退回 NetApp ，以完成 AFF C80 控制器更換工作流程的最後步驟。 
---
= 完整控制器更換 - AFF C80
:allow-uri-read: 
:icons: font
...
[role="lead"]
驗證邏輯介面是否正在回報其主連接埠，執行叢集健全狀況檢查，並將故障零件退回 NetApp ，以完成 AFF C80 控制器更換工作流程的最後步驟。
