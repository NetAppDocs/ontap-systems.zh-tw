...
summary: 在 AFF C80 系統中的替換開機媒體上還原加密，以確保資料持續受到保護。此程序包括驗證金鑰可用度，重新套用加密設定，以及確認資料的安全存取。 
---
= 還原加密 - AFF C80
:allow-uri-read: 
:icons: font
...
[role="lead"]
在 AFF C80 系統中的替換開機媒體上還原加密，以確保資料持續受到保護。此程序包括驗證金鑰可用度，重新套用加密設定，以及確認資料的安全存取。
