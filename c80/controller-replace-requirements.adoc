...
summary: 在更換 AFF C80 系統中的控制器之前，請確保您符合成功更換所需的要求。這包括擁有正確的更換控制器，系統組態備份，以及存取任何必要的工具和韌體。 
---
= 更換控制器的要求 - AFF C80
:allow-uri-read: 
:icons: font
...
[role="lead"]
在更換 AFF C80 系統中的控制器之前，請確保您符合成功更換所需的要求。這包括擁有正確的更換控制器，系統組態備份，以及存取任何必要的工具和韌體。
...
.接下來呢？
在您審查了更換 AFF C80 控制器的要求之後，您需要link:controller-replace-shutdown.html["關閉控制器"]。
