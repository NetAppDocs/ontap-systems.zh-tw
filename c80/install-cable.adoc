...
[role="lead"]
為 AFF C80 儲存系統安裝機架硬體後、請安裝控制器的網路纜線、並將纜線連接到控制器和儲存櫃。
...
* AFF C80 控制器上的 I/O 插槽編號為 1 至 11 。
+
...
.交換式叢集纜線
--
使用 100 GbE 電纜將控制器 A 和 B 的 e1a 和 e7a 連接埠連接到叢集網路交換器 A 和 B。
...
NOTE: 請勿使用連接埠 e1b 和 e7b 進行主機網路連線。請使用獨立的主機網路卡。
...
.關於這項工作
AFF C80 儲存系統支援配備 NSM100 或 NSM100B 模組的 NS224 機櫃。這兩個模組的主要差異在於：
...
.. 將連接埠 e11b 連接到機櫃 2，NSM A 連接埠 e0b。
+
...
====
.接下來呢？
為 AFF C80 系統連接硬體之後，您link:install-power-hardware.html["開啟 AFF C80 儲存系統電源"]就可以了。
