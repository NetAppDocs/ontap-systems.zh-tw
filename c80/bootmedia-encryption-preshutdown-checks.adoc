...
summary: 為確保AFF C80 儲存系統上的資料安全，您需要驗證啟動媒體上的加密金鑰支援情況及其狀態。請檢查您的ONTAP版本是否支援NetApp磁碟區加密，並在關閉控制器之前檢查金鑰管理員是否處於活動狀態。 
---
= 檢查 AFF C80 儲存系統中開機媒體的加密支援
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
為了確保 AFF C80 儲存系統上的資料安全性，您需要驗證開機媒體上的加密金鑰支援和狀態。檢查您的 ONTAP 版本是否支援 NetApp Volume Encryption （ NVE ），以及在關閉控制器之前，檢查金鑰管理程式是否為作用中。
