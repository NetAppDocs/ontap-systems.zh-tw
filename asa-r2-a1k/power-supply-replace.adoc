...
summary: 當您的 ASA A1K 系統中的交流或直流電源單元發生故障或損壞時，請更換此電源單元，以確保系統持續獲得穩定運作所需的電力。更換過程包括：斷開故障電源單元與電源的連接、拔除電源線、更換故障電源單元，然後重新連接電源。 
---
= 在 ASA A1K 系統中熱交換電源供應單元
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當 ASA A1K 系統故障或故障時，請更換其中的 AC 或 DC 電源供應器（ PSU ），以確保您的系統能繼續接收所需的電力，以便穩定運作。更換程序包括中斷故障 PSU 與電源的連接，拔下電源線，更換故障 PSU ，然後將其重新連接至電源。
