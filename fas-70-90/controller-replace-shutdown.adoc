...
summary: 當 FAS70 或 FAS90 儲存系統的控制器受損時，請將其關閉。在執行維護或元件更換之前，請務必安全關閉控制器，以避免資料遺失並確保系統穩定性。 
---
...
[role="lead"]
當 FAS70 或 FAS90 儲存系統的控制器受損時，請將其關閉。在執行維護或元件更換之前，請務必安全關閉控制器，以避免資料遺失並確保系統穩定性。
...
====
.接下來呢？
關閉控制器之後，您需要執行link:controller-replace-move-hardware.html["更換控制器"]。
