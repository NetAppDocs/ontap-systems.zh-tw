...
summary: 更換 FAS70 或 FAS90 系統中的即時時鐘 RTC 電池（通常稱為鈕扣電池），以確保依賴精確時間同步的服務和應用程式能夠繼續運作。 
---
= 更換 FAS70 或 FAS90 系統中的即時時鐘電池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
更換 FAS70 或 FAS90 系統中的實時時鐘（ RTC ）電池（通常稱爲幣形電池），以確保依賴準確時間同步的服務和應用程序保持正常運行。
