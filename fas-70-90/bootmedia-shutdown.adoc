...
summary: 關閉 FAS70 或 FAS90 儲存系統中受損的控制器，以防止資料遺失並在手動啟動媒體復原過程中保持系統穩定性。 
---
= 在手動開機恢復過程中，關閉 FAS70 或 FAS90 系統中發生故障的控制器。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
關閉 FAS70 或 FAS90 儲存系統中受損的控制器，以防止資料遺失並在手動啟動媒體復原過程中保持系統穩定性。
