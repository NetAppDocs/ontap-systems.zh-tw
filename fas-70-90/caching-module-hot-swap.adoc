...
---
= 更換 FAS70 或 FAS90 系統中的 Flash Cache 模組業者或快取模組
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.開始之前
* 請確認您的儲存系統是否具有適合替換 Flash Cache 模組業者的作業系統。
* 確認其他所有組件均運作正常。如果發生故障，請聯絡技術支援。
...
.開始之前
* 確認替換的快取模組與故障模組的容量相同，可以是同一廠商的產品，也可以是其他受支援廠商的產品。
* 確認其他所有組件均運作正常。如果發生故障，請聯絡技術支援。
...
.. 轉動凸輪把手至最遠的位置。
.. 將手指勾入凸輪桿開口，然後將快取模組從 Flash Cache 模組載體中拉出，即可從機箱中取出快取模組。
+
...
+
確保命令輸出顯示狀態為 `powered-on`且已準備好運作。
