...
summary: NVMe SSD FlashCache 模組承載器包含一或兩個 FlashCache 模組（快取模組）、每個快取模組均整合一個 SSD 快取磁碟機。快取模組可以熱插拔、但 FlashCache 模組承載器不是可熱交換的。 
---
= 更換 FlashCache 模組載體或快取模組： FAS70 和 FAS90
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
NVMe SSD FlashCache 模組承載器包含一或兩個 FlashCache 模組（快取模組）、每個快取模組均整合一個 SSD FlashCache 磁碟機。

FAS70 支援 2TB 快取模組、 FAS90 支援 4TB 快取模組。您無法在 FlashCache 模組載體中混合使用容量不同的快取模組。

根據需要更換的項目、您可以執行下列任一程序：整個 Flashcache 模組載體或快取模組。
...
FlashCache 模組承載器位於插槽 6 中、最多可容納兩個 FlashCache 模組。您無法熱交換 FlashCache 模組載體

.開始之前
* 確保您的儲存系統擁有適當的作業系統、可用於更換 FlashCache 模組承載器。
...
=== 步驟 2 ：更換 FlashCache 模組承載器

請執行下列步驟以更換 FlashCache 模組承載器。

.步驟
. 如果您尚未接地、請正確接地。
. 在插槽 6 中找到故障的 FlashCache 模組承載器、其位置是 FlashCache 模組承載器正面的琥珀色警示 LED 。
+
image::../media/drw_fas70-90_remove_caching_module_carrier_ieops-1772.svg[移除 FlashCache 模組載體]
...
快取模組插槽編號
 ...
 a| 
image:../media/legend_icon_03.svg["編號 3 、寬度 =30px"]
 a| 
FlashCache 模組承載器 CAM 把手



 a| 
image:../media/legend_icon_04.svg["編號 4 、寬度 =30px"]
 a| 
FlashCache 模組載體故障 LED

|===
. 移除故障的 FlashCache 模組載體：
+
.. 向下轉動纜線管理承載器、方法是拉動纜線管理承載器內側兩側的按鈕、然後向下旋轉承載器。
.. 捏住 FlashCache 模組載體底部的藍色彈片。
.. 將彈片從模組轉開。


. 將 FlashCache 模組承載器從控制器模組中拉出、並將其放在防靜電墊上。
. 將快取模組移至替換的 FlashCache 模組載體：
+
.. 捏住快取模組頂端的 Terra Cotta 標籤、將 CAM 把手轉離快取模組。
.. 將手指插入 CAM 拉桿開口處、然後將模組從 FlashCache 模組承載器中拉出、即可將模組從機箱中移除。
.. 將快取模組安裝到替換 FlashCache 模組承載器的同一個插槽中、然後將 CAM 把手旋轉到快取模組上的關閉位置、將其鎖定到位。


. 如果有第二個快取模組、請重複這些步驟。
. 將替換的 FlashCache 模組承載器安裝到系統中：
+
.. 將模組與機箱插槽開口的邊緣對齊。
...
更換 FlashCache 模組承載器之後、您必須重新啟動控制器模組。
...
.開始之前
FlashCache 模組（快取模組）位於插槽 6-1 或插槽 6-2 中、或插槽 6-1 和插槽 6-2 中。
...
.. 轉動凸輪把手至最遠的位置。
.. 將手指插入 CAM 槓桿開口處、然後將模組從 FlashCache 模組承載器中拉出、即可將快取模組模組模組從機箱中移除。
+
從 FlashCache 模組載體中移除快取模組時、請務必支援快取模組。
