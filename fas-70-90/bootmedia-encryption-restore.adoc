...
summary: 在 FAS70 或 FAS90 系統中的替換開機媒體上還原加密，以確保資料持續受到保護。更換程序包括驗證金鑰可用度，重新套用加密設定，以及確認資料的安全存取。 
---
= 手動開機恢復程序完成後，恢復 FAS70 或 FAS90 系統中開機媒體上的加密。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 FAS70 或 FAS90 系統中的替換開機媒體上還原加密，以確保資料持續受到保護。更換程序包括驗證金鑰可用度，重新套用加密設定，以及確認資料的安全存取。
