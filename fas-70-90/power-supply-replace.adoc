...
summary: 當您的 FAS70 或 FAS90 系統中的交流或直流電源供應器單元 PSU 發生故障或損壞時，請更換此電源供應器單元，以確保系統持續獲得穩定運作所需的電力。更換過程包括：斷開故障 PSU 與電源的連接、拔除電源線、更換故障 PSU，然後重新連接電源。 
---
= 在 FAS70 或 FAS90 系統中熱插拔電源供應單元
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.關於這項工作
本程序適用於一次更換一個 PSU。請根據您的 PSU 類型使用適當的程序：AC 或 DC。
