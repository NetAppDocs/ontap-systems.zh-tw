...
summary: 在更換 FAS70 或 FAS90 系統中的控制器之前，請確認您符合成功更換的必要條件。這包括驗證系統中的所有其他元件是否正常運作、驗證您擁有正確的替換控制器，以及將控制器的主控台輸出儲存到文字日誌檔。 
---
= 在更換 FAS70 或 FAS90 系統中的控制器之前，請先檢視相關要求
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在更換 FAS70 或 FAS90 系統中的控制器之前，請確認您符合成功更換的必要條件。這包括驗證系統中的所有其他元件是否正常運作、驗證您擁有正確的替換控制器，以及將控制器的主控台輸出儲存到文字日誌檔。
...
.接下來呢？
在您審查了更換 FAS70 或 FAS90 控制器的要求之後，您需要link:controller-replace-shutdown.html["關閉受損的控制器"]。
