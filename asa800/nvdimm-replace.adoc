...
summary: 當您的 ASA A800 系統偵測到快閃記憶體壽命即將到期或偵測到的 NVDIMM 記憶體狀況不佳時，請更換 NVDIMM 記憶體條。否則，系統將發生崩潰。 
---
= 更換 ASA A800 系統中的 NVDIMM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當您的ASA A800系統偵測到快閃記憶體壽命即將結束或偵測到已識別的 NVDIMM 整體狀況不佳時，請更換系統中的 NVDIMM；否則會導致系統崩潰。

.開始之前
* 請確保您已收到從NetApp收到的替換 NVDIMM，並且該 NVDIMM 與您的ASA A800系統相容。
