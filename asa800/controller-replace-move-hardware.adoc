...
[role="lead"]
當硬體故障需要更換時，請更換ASA A800系統中的控制器。更換過程包括移除受損的控制器、將組件移至更換的控制器、安裝更換的控制器以及重新啟動它。
...
.接下來呢？
更換故障的ASA A800控制器後，您需要：link:controller-replace-system-config-restore-and-verify.html["還原系統組態"]
