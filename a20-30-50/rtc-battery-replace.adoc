...
summary: 更換 AFF A20、AFF A30 或 AFF A50 儲存系統中的即時時鐘電池（通常稱為鈕扣電池），以確保依賴精確時間同步的服務和應用程式能夠繼續運作。 
---
= 更換 AFF A20、AFF A30 或 AFF A50 系統中的即時時鐘電池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: 更換 RTC 電池後，插入控制器，並啟動系統進行首次 BIOS 重置，您將看到以下錯誤訊息：
...
NOTE: 如果您的儲存系統在開機功能表處停止，請選擇該選項 `Reboot node`並在提示時回答 _y_，然後按 _Ctrl-C_ 開機至 LOADER。
...
. 在 LOADER 提示字元下，輸入 `bye`以重新初始化 I/O 模組和其他元件，並重新開機控制器。
