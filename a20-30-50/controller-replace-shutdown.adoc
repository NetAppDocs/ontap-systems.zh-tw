...
summary: 關閉 AFF A20 ， AFF A30 或 AFF A50 儲存系統中受損的控制器，以防止資料遺失，並確保更換控制器時系統穩定。 
---
= 更換 AFF A20、AFF A30 或 AFF A50 系統中發生故障的控制器之前，請先將其關閉。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
關閉 AFF A20 ， AFF A30 或 AFF A50 儲存系統中受損的控制器，以防止資料遺失，並確保更換控制器時系統穩定。
...
====
.接下來呢？
關閉受損控制器後link:controller-replace-move-hardware.html["更換控制器"]，您需要執行。
