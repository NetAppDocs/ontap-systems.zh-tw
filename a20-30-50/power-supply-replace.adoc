...
summary: 當您的 AFF A20、AFF A30 或 AFF A50 儲存系統中的交流電或直流電源供應器故障或發生故障時，請更換電源供應器，以確保系統持續獲得穩定運作所需的電力。更換過程包括：斷開故障 PSU 與電源的連接、拔除電源線、更換故障 PSU，然後重新連接電源。 
---
= 在 AFF A20、AFF A30 或 AFF A50 系統中熱交換的電源供應單元
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當AFF A20、 AFF A30 或AFF A50 儲存系統中的交流或直流電源裝置 (PSU) 發生故障或故障時，請更換此電源裝置，以確保系統繼續接收穩定運作所需的電力。更換過程包括將故障的 PSU 與電源斷開、拔除電源線、更換故障的 PSU，然後將其重新連接到電源。
