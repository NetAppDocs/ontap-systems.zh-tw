...
summary: 將 I/O 模組新增至 AFF A20 ， AFF A30 或 AFF A50 系統，以增強網路連線能力，並擴充系統處理資料流量的能力。 
---
= 在 AFF A20、AFF A30 或 AFF A50 系統新增 I/O 模組
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在您的AFF A20、 AFF A30 或AFF A50 儲存系統中新增 I/O 模組，以增強網路連接並擴展系統處理資料流量的能力。

新增 I/O 模組需要關閉一個控制器、新增模組並重新開機，然後對另一個控制器重複這些步驟。

.關於這項工作
* 當有可用插槽或所有插槽都已滿時，您可以將 I/O 模組新增至儲存系統。
...
+
您可以透過輸入 `system location-led _off_`命令來關閉它們。如果您不確定 LED 燈是亮著還是熄滅，可以透過輸入 `system location-led show`命令來檢查它們的狀態。
