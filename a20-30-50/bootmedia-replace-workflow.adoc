...
summary: 啟動映像的手動復原涉及使用 USB 磁碟機將ONTAP重新安裝到AFF A20、 AFF A30 或AFF A50 儲存系統的替換啟動媒體上。您必須從NetApp支援網站下載對應的ONTAP復原映像並將其複製到 USB 隨身碟。然後，使用準備好的 USB 隨身碟執行恢復操作，將系統恢復到正常運作狀態。 
---
= 使用手冊開機恢復流程更換 AFF A20、AFF A30 和 AFF A50 開機媒體的工作流程
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
啟動映像的手動復原涉及使用 USB 磁碟機將ONTAP重新安裝到AFF A20、 AFF A30 或AFF A50 儲存系統的替換啟動媒體上。您必須從NetApp支援網站下載對應的ONTAP復原映像並將其複製到 USB 隨身碟。然後，使用準備好的 USB 隨身碟執行恢復操作，將系統恢復到正常運作狀態。
