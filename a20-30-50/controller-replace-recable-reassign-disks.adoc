...
summary: 將儲存資源的控制權交還給替換控制器，以便您的 AFF A20、AFF A30 或 AFF A50 儲存系統能夠恢復正常運作。交還控制權的步驟會根據您的系統是否使用加密（無加密、Onboard Key Manager 加密或 External Key Manager 加密）而有所不同。 
---
= 將 AFF A20、AFF A30 或 AFF A50 系統中的控制器交還給系統，即可恢復正常運作。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
