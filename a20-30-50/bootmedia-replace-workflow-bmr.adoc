...
summary: 啟動映像的自動恢復涉及系統自動識別並選擇適當的啟動選單選項。它使用合作夥伴節點上的啟動映像在AFF A20、 AFF A30 或AFF A50 儲存系統中的替換啟動媒體上重新安裝ONTAP 。 
---
= 使用自動化開機恢復流程的 AFF A20、AFF A30 和 AFF A50 開機媒體更換工作流程
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
啟動映像的自動恢復涉及系統自動識別並選擇適當的啟動選單選項。它使用合作夥伴節點上的啟動映像在AFF A20、 AFF A30 或AFF A50 儲存系統中的替換啟動媒體上重新安裝ONTAP 。

自動啟動媒體復原過程僅在ONTAP 9.17.1 及更高版本中支援。如果您的儲存系統運行的是早期版本的ONTAP，請使用link:bootmedia-replace-workflow.html["手動啟動恢復程序"] 。

首先，審查更換需求、關閉控制器、更換開機媒體、還原映像，並驗證系統功能。
