...
summary: 若要將系統還原為完整作業，您必須還原 NetApp Storage Encryption 組態、為新控制器安裝授權、並依照套件隨附的 RMA 指示將故障零件退回 NetApp。 
---
= 完成 AFF C190 系統的控制器更換
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
完成 AFF C190 系統的控制器更換，方法是還原 NetApp 儲存加密組態（如有必要），安裝新控制器的授權，並將故障零件退回 NetApp。



[[step-1-installing-licenses-for-the-replacement-controller-in-ontap]]
== 步驟1：在ONTAP 更新版本中安裝_replace_控制器的授權
