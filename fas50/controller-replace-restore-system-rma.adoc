...
summary: 若要完成 FAS50 系統的控制器更換、請先視需要還原 NetApp Storage Encryption 組態、然後在新控制器上安裝所需的授權。接下來、請確認邏輯介面正在向其主連接埠報告、並執行叢集健全狀況檢查。最後、請登錄新控制器的序號、然後將故障零件退回 NetApp。 
---
= 完整更換控制器： FAS50
:allow-uri-read: 
:icons: font
...
[role="lead"]
若要完成 FAS50 系統的控制器更換，請先還原 NetApp 儲存加密組態（如有必要），然後在新控制器上安裝所需的授權。接著，確認邏輯介面（生命）正在向其主連接埠回報，並執行叢集健全狀況檢查。最後，請登錄新控制器的序號，然後將故障零件退回 NetApp 。
