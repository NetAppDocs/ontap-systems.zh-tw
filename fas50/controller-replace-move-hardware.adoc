...
summary: 當硬體故障需要更換 FAS50 儲存系統中的控制器。更換過程包括移除受損的控制器、將組件移至更換的控制器、安裝更換的控制器以及重新啟動它。 
---
= 更換控制器 - FAS50
:allow-uri-read: 
:icons: font
...
[role="lead"]
當硬體故障需要更換 FAS50 儲存系統中的控制器。更換過程包括移除受損的控制器、將組件移至更換的控制器、安裝更換的控制器以及重新啟動它。
...
CAUTION: 在關閉故障控制器護蓋之前，請確保所有空 DIMM 插槽上的退出卡榫已完全回復至鎖定位置。
...
.接下來呢？
更換功能受損的 FAS50 控制器後link:controller-replace-system-config-restore-and-verify.html["還原系統組態"]，您需要。
