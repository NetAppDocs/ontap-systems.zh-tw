...
summary: 如果偵測到過多可修正或不可修正的記憶體容量錯誤，請更換 FAS50 儲存系統中的 DIMM。此類錯誤可能導致儲存系統無法開機 ONTAP。更換過程包括：關閉受損控制器，將其拆下，更換 DIMM，重新安裝控制器，然後將故障部件退回 NetApp。 
---
= 更換 FAS50 系統中的記憶體條以修復記憶體容量錯誤
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如果偵測到過多可修正或不可修正的記憶體容量錯誤，請更換 FAS50 儲存系統中的 DIMM。此類錯誤可能導致儲存系統無法開機 ONTAP。更換過程包括：關閉受損控制器，將其拆下，更換 DIMM，重新安裝控制器，然後將故障部件退回 NetApp。
