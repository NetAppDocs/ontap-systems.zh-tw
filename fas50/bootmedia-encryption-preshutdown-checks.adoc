...
summary: 為了確保 FAS50 儲存系統的資料安全性，您需要驗證開機媒體上的加密金鑰支援和狀態。檢查您的 ONTAP 版本是否支援 NetApp Volume Encryption （ NVE ），以及在關閉控制器之前，檢查金鑰管理程式是否為作用中。 
---
= 使用手冊開機恢復流程檢查 FAS50 系統中開機媒體的加密支援
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
為確保 FAS50 儲存系統的資料安全，您需要驗證開機媒體上的加密金鑰支援和狀態。檢查您的 ONTAP 版本是否支援 NetApp 磁碟區加密 （NVE），並在關閉控制器之前檢查金鑰管理程式是否處於作用中。
