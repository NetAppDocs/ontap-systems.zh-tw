...
summary: 在 FAS50 儲存系統中安裝新的啟動媒體設備後，您可以啟動自動啟動媒體復原流程，從合作夥伴節點復原設定。 
---
= 使用自動開機恢復流程在 FAS50 系統上啟動恢復映像
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 FAS50 儲存系統中安裝新的啟動媒體設備後，您可以啟動自動啟動媒體復原過程，以從配對節點復原配置。在復原過程中，系統會檢查是否已啟用加密，並決定正在使用的金鑰加密類型。如果已啟用金鑰加密，系統將引導您完成對應的復原步驟。
