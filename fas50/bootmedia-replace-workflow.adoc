...
summary: 手動恢復啟動映像需要使用 USB 隨身碟將ONTAP重新安裝到 FAS50 系統的替換啟動媒體上。您必須從NetApp支援網站下載對應的ONTAP復原映像並將其複製到 USB 隨身碟。然後，使用準備好的 USB 隨身碟執行恢復操作，將系統恢復到正常運作狀態。 
---
= 使用手冊開機恢復流程的 FAS50 開機媒體更換工作流程
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
手動恢復啟動映像需要使用 USB 隨身碟將ONTAP重新安裝到 FAS50 系統的替換啟動媒體上。您必須從NetApp支援網站下載對應的ONTAP復原映像並將其複製到 USB 隨身碟。然後，使用準備好的 USB 隨身碟執行恢復操作，將系統恢復到正常運作狀態。
