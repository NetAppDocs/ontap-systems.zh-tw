...
summary: 更換控制器時，請關閉 ASA C400 儲存系統中的控制器，以防止資料遺失並確保系統穩定性。 
---
= 更換 ASA C400 系統中發生故障的控制器之前，請先將其關機。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
更換控制器時，請關閉 ASA C400 儲存系統中的控制器，以防止資料遺失並確保系統穩定性。
...
====
.接下來呢？
關閉 ASA C400 控制器後，您需要link:controller-replace-move-hardware.html["更換控制器"]。
