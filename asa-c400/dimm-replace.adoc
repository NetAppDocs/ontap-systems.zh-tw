...
summary: 當系統登錄越來越多的可修正錯誤修正碼（correctable error correction codes）或不可修正的 ECC 錯誤時，請更換 ASA C400 控制器模組中的 DIMM。若未更換故障的 DIMM，可能會導致系統出現緊急狀況（panic）。 
---
= 更換 ASA C400 系統中的 DIMM 以修復記憶體容量錯誤
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當儲存系統遇到過多的可修正錯誤校正碼 (ECC) 錯誤或無法修正的 ECC 錯誤時，請更換 ASA C400 系統中的 DIMM，這通常是由單一 DIMM 故障所引起的。
