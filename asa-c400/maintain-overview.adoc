...
summary: 維護ASA C400儲存系統的硬件，以確保長期可靠性和最佳效能。定期執行維護任務（例如更換故障組件），有助於防止停機和資料遺失。 
---
= 了解如何維護您的 ASA C400 儲存系統
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
維護ASA C400儲存系統的硬件，以確保長期可靠性和最佳效能。定期執行維護任務（例如更換故障組件），有助於防止停機和資料遺失。

維護過程假定ASA C400儲存系統已部署為ONTAP環境中的儲存節點。
...
對於 ASA C400 儲存系統、您可以對下列元件執行維護程序。
