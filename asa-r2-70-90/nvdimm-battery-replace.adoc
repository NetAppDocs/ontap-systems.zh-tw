...
---
= 更換 ASA A70 或 ASA A90 系統中的 NV 電池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
關閉或接管故障控制器。接管並停止故障控制器，以便正常控制器繼續從故障控制器的儲存提供資料。為此，您需要在 AutoSupport 中停用自動建立案例，停用自動恢復，並將故障控制器置於 LOADER 提示字元。LOADER 提示符號是安全的停止狀態，您可以從中更換 FRU。
