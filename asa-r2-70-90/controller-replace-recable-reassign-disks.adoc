...
summary: 將儲存資源的控制權交給更換控制器，讓 ASA A70 或 ASA A90 系統恢復正常運作。 
---
= 將 ASA A70 或 ASA A90 系統中的控制器交還，以恢復正常運作。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
將儲存資源的控制權交給更換控制器，讓 ASA A70 或 ASA A90 系統恢復正常運作。復原過程會根據系統使用的加密類型而有所不同：無加密或板載金鑰管理員 (OKM) 加密。

[role="tabbed-block"]
====
.無加密或 External Key Manager（EKM）
--
...
.機載加密（ OKM ）
--
還原並重新密封 OKM 的內建加密金鑰，並使控制器恢復正常運作。

.開始之前
開始之前，請先檢閱 https://docs.netapp.com/us-en/ontap-cli/security-key-manager-onboard-show-backup.html["security key-manager onboard show-backup"]或聯絡 NetApp 支援部門。

.步驟
. 從 Loader 提示字元開機至 ONTAP 功能表 `boot_ontap menu`、然後選取選項 10 。
. 顯示叢集的金鑰管理備份資訊。
+
.. 執行以下命令以顯示叢集的金鑰管理備份資訊：
+
...
.. 驗證金鑰狀態： `security key-manager key query -restored false`
+
正確同步後，輸出結果不應顯示任何內容。
...
NOTE: 請勿推翻對遺失金鑰的還原否決權。請聯絡 NetApp 支援部門。
