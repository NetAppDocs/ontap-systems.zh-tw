...
summary: 更換 ASA A70 或 ASA A90 系統中的即時時鐘電池（通常稱為鈕扣電池），以確保依賴精確時間同步的服務和應用程式保持運作。 
---
= 更換 ASA A70 或 ASA A90 系統中的即時時鐘電池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
請在 ASA A70 或 ASA A90 系統中更換即時時鐘（ RTC ）電池（通常稱為幣式電池），以確保仰賴精確時間同步的服務和應用程式仍能正常運作。
