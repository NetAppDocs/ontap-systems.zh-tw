...
summary: 若要恢復 AFF A800 系統的完整運行，如有必要，請恢復 NetApp 儲存加密配置，安裝新控制器的授權、並將故障零件退回 NetApp。 
---
= 完成 AFF A800 系統的控制器更換
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
若要完成 AFF A800 系統的控制器更換，還原 NetApp Storage Encryption 組態（如有必要），確認邏輯介面 （LIF） 正在向其主連接埠報告，執行叢集資訊健檢，並將故障元件退回 NetApp。
