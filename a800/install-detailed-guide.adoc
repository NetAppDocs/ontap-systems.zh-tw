...
[role="lead"]
請按照這些詳細的逐步說明安裝您的 AFF A800 儲存系統，從準備站點到啟動 ONTAP 叢集。
...
您的平台叢集需要佈線，可以選擇使用雙節點無交換器式叢集方式或叢集互連網路方式。此外，也可以選擇佈線連接到 Fibre Channel 或 iSCSI 主機網路或直連式儲存設備。這些佈線並非互斥，您可以同時連接主機網路和儲存設備。
