...
summary: 當系統偵測到快閃記憶體壽命即將耗盡或偵測到的 NVDIMM 記憶體條發生故障時，請更換 AFF A800 控制器模組中的 NVDIMM 記憶體條。未更換故障的 NVDIMM 記憶體條可能會導致系統崩潰。 
---
= 更換 AFF A800 系統中的 NVDIMM 記憶體條
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當您的 AFF A800 系統偵測到快閃記憶體壽命即將結束或偵測到的 NVDIMM 發生故障時，請更換 NVDIMM。如果未更換故障的 NVDIMM，會導致系統崩潰。

.開始之前
* 請確保您已從 NetApp 收到替換的 NVDIMM，並且它與您的 AFF A800 系統相容。
* 請確保儲存系統中的所有其他元件均運作正常。如果發生故障，請聯絡 NetApp 支援。
...
使用風管頂部的 NVDIMM 定位標籤找到控制器模組中的 NVDIMM，然後進行更換。
