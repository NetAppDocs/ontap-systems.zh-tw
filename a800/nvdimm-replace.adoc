...
[role="lead"]
當您的AFF A800系統偵測到快閃記憶體壽命即將結束或偵測到已識別的 NVDIMM 記憶體狀況不佳時，請更換 NVDIMM 記憶體條；否則會導致系統崩潰。

.開始之前
* 請確保您已收到從NetApp收到的替換 NVDIMM，並且該 NVDIMM 與您的AFF A800系統相容。
* 請確保儲存系統中的所有其他元件均正常運作；如果出現問題，請聯絡NetApp支援。
