...
---
= 重新連接 AFF A800 系統中的系統線纜並重新分配磁碟機
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
更換控制器後，請重新連接 AFF A800 系統的線纜，並將磁碟重新指派給更換後的控制器。如果您的系統採用 HA 配對，則歸還程序會自動將系統 ID 指派給磁碟；具體步驟取決於您的系統是否使用無加密或 Onboard Key Manager （OKM）加密。
...
如果儲存系統採用 HA 配對，則在程序結束時進行歸還時，新控制器模組的系統 ID 會自動指派給磁碟機。您必須在開機_replacement_ 控制器時確認系統 ID 變更，然後驗證系統是否已套用此變更。

此步驟僅適用於在 HA 配對中執行 ONTAP 的系統。
