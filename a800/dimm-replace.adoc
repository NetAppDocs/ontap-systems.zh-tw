...
summary: 當系統偵測到越來越多的可修正錯誤修正代碼或不可修正的 ECC 錯誤時,請更換 AFF A800 控制器模組中的 DIMM 記憶體條。未更換故障的 DIMM 記憶體條可能會導致系統崩潰。 
---
= 更換 AFF A800 系統中的 DIMM 以修復記憶體容量錯誤
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當儲存系統遇到過多可修正 （CECC） 或不可修正的記憶體容量錯誤時，請更換 AFF A800 系統中的 DIMM 記憶體條。此類錯誤可能導致儲存系統無法開機 ONTAP。

.開始之前
* 請確保系統中所有其他組件都運作正常。如果它們運作不正常，請聯絡技術支援。
