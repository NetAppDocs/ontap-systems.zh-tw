...
summary: 在雙節點 MetroCluster 組態中完成 FRU 更換後、您可以執行 MetroCluster 切換回作業。這會將組態恢復正常作業狀態、先前受損站台上的同步來源儲存虛擬機器（ SVM ）現在處於作用中狀態、並從本機磁碟集區提供資料。 
---
= 在您的 ASA A400 系統中，切換回雙節點 MetroCluster 組態中的集合體。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
