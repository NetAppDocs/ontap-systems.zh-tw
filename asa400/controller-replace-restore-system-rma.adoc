...
summary: 完成 ASA A400 系統的控制器更換。安裝替換控制器的授權、確認邏輯介面（LIF）正在向其主連接埠報告、並執行叢集健全狀況檢查。最後、將故障零件退回 NetApp。 
---
= 完成 ASA A400 系統的控制器更換
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
完成 ASA A400 系統的控制器更換。安裝替換控制器的授權、確認邏輯介面（LIF）正在向其主連接埠報告、並執行叢集健全狀況檢查。最後、將故障零件退回 NetApp。
