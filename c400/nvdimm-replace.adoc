...
summary: 當系統偵測到快閃記憶體壽命即將結束或偵測到已識別的 NVDIMM 整體狀況不佳時，必須更換控制器模組中的 NVDIMM，否則會導致系統崩潰。 
---
= 更換 AFF C400 儲存系統中的 NVDIMM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當 AFF C400 儲存系統出現過多的可修正 ECC 錯誤或不可修正 ECC 錯誤時，請更換 NVDIMM，這通常是由單一 NVDIMM 故障所引起的。
