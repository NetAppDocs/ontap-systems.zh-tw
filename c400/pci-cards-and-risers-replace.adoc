...
[role="lead"]
若要更換 AFF C400 儲存系統中的 PCIe 或夾層卡，請拔除連接至卡片的纜線和 SFP/QSFP 模組，更換故障卡，然後重新連接纜線。
...
NOTE: 務必將夾層卡上的蝶形螺絲完全擰緊。如果螺絲鬆動，可能無法偵測到連接埠 e0e 至 e0h，LED 指示燈可能會持續閃爍，且日誌中可能會顯示「不支援的電纜」錯誤。
