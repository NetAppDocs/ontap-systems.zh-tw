...
summary: 更換控制器時，請關閉 AFF C400 儲存系統中的控制器，以防止資料遺失並確保系統穩定性。 
---
= 更換 AFF C400 儲存系統中的受損控制器之前，請先將其關閉。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
更換控制器時，請關閉 AFF C400 儲存系統中的控制器，以防止資料遺失並確保系統穩定性。
...
====
.接下來呢？
關閉 AFF C400 控制器後，您需要執行下列操作link:controller-replace-move-hardware.html["更換控制器"]。
