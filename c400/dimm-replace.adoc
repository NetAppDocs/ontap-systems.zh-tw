...
summary: '當系統登錄越來越多的可修正錯誤修正代碼 (ECC) 時，您必須更換控制器模組中的 DIMM，否則會導致系統出現問題。' 
---
= 更換 AFF C400 儲存系統中的 DIMM 以修復記憶體容量錯誤
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當 AFF C400 儲存系統遇到過多的可修正 ECC 錯誤或不可修正 ECC 錯誤時，請更換 DIMM，這通常是由單一 DIMM 故障所引起的。
