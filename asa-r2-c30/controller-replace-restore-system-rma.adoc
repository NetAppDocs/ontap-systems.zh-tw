...
summary: 若要完成 ASA C30 儲存系統的控制器更換，首先如有必要，請還原 NetApp Storage Encryption 配置，並在新控制器上安裝所需的授權。接下來，確認邏輯介面正在向其主連接埠報告，並執行叢集健全狀況檢查。最後，註冊新控制器的序號，然後將故障零件退回 NetApp。 
---
= 完整控制器更換 - ASA C30
:allow-uri-read: 
:icons: font
...
[role="lead"]
若要完成 ASA C30 儲存系統的控制器更換，請先恢復 NetApp 儲存加密配置（如有必要）並在新控制器上安裝所需的許可證。接著，確認邏輯介面（生命）正在向其主連接埠回報，並執行叢集健全狀況檢查。最後，請登錄新控制器的序號，然後將故障零件退回 NetApp 。
