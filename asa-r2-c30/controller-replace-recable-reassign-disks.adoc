...
summary: 將儲存資源的控制權交還給替換控制器，以便您的 ASA C30 儲存系統能夠恢復正常運作。交還控制權的步驟會根據您系統使用的加密類型而有所不同，包括無加密、Onboard Key Manager 加密或 External Key Manager 加密。 
---
= 將控制器交還給您的 ASA C30 系統，以恢復正常運作。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
將儲存資源的控制權交還給替換控制器，以便您的 ASA C30 儲存系統可以恢復正常運作。回饋程序會因系統使用的加密類型而異：無加密，內建金鑰管理程式（ OKM ）加密或外部金鑰管理程式（ EKM ）加密。
