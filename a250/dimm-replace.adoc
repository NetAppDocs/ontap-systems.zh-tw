...
summary: 當系統根據健康監視器警報遇到過多的 CECC 錯誤，或遇到無法修正的 ECC 錯誤導致儲存系統無法啟動 ONTAP 時，請更換 AFF A250 控制器中的 DIMM。 
---
= 更換 AFF A250 系統中的 DIMM 以修復記憶體容量錯誤
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當儲存系統遇到過多可修正（CECC）或不可修正的記憶體容量錯誤時，請更換 AFF A250 系統中的 DIMM。此類錯誤可能導致儲存系統無法開機 ONTAP。
