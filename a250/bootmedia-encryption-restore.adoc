...
summary: 在 AFF A250 系統中，恢復替換開機媒體上的加密。完成針對已啟用 Onboard Key Manager、NetApp Storage Encryption 或 NetApp Volume Encryption 的系統的特定步驟。 
---
= 在手動開機恢復程序之後，還原 AFF A250 系統中開機媒體上的加密。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
請在 AFF A250 系統中，使用替換開機媒體恢復加密，以確保資料持續受到保護。替換程序包括驗證金鑰可用性、重新套用加密設定以及確認對資料的安全存取。
...
====
.接下來呢？
在恢復開機媒體上的加密後，link:bootmedia-complete-rma.html["將故障零件退回 NetApp"]。
