...
[role="lead"]
請按照這些詳細的逐步說明安裝您的 AFF A250 儲存系統，從準備站台到啟動 ONTAP 叢集。
...
.步驟
. 使用 25 GbE 叢集互連纜線將叢集互連連接埠 e0c 連接至 e0c，並將 e0d 連接至 e0d。
+
...
NOTE: link:https://hwu.netapp.com["NetApp Hardware Universe"^]主機網路卡（Fibre Channel 或 25 GbE）的插槽優先權為插槽 2。但是，如果您同時擁有這兩種網路卡，則 Fibre Channel 網路卡應安裝在插槽 2，25 GbE 網路卡應安裝在插槽 1（如下方選項所示）。如果您使用外部儲存架，儲存卡應安裝在插槽 1，因為插槽 1 是唯一支援儲存架的插槽。
...
--
.方案二：透過電纜連接至 25 GbE 資料或主機網路
--
控制器上的 25 GbE 連接埠連接到 25 GbE 資料或主機網路交換器。
...
. 纜線控制器 A 至機櫃。
+
image:../media/drw_affa250_1shelf_cabling_a.png["控制器 1 纜線"]

. 纜線控制器 B 至機櫃。
+
image:../media/drw_affa250_1shelf_cabling_b.png["控制器 2 纜線"]
...
NOTE: 有關如何建立控制器控制台連接埠的持續連接以進行遠端管理的信息，請參閱知識庫文章。 https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/How_to_connect_an_ONTAP_controller_console_port_for_remote_management["如何連接ONTAP控制器控制台連接埠以進行遠端管理"^]。
