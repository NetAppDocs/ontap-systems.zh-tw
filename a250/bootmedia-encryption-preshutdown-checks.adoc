...
---
= 使用手冊開機恢復流程檢查 AFF A250 系統中開機媒體的加密支援
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
為確保 AFF A250 儲存系統的資料安全，請驗證開機媒體上的加密金鑰支援情況和狀態。檢查您的 ONTAP 版本是否支援 NetApp Volume Encryption （NVE）。關閉控制器前，請檢查金鑰管理程式是否已作用中。

如果您的儲存系統執行的是 ONTAP 9.18.1 或更新版本，請使用link:bootmedia-replace-workflow-bmr.html["自動啟動恢復程序"]。如果您的系統執行的是較早版本的 ONTAP，則必須使用手動開機還原程序。
...
.接下來呢？
檢查開機媒體上的加密金鑰支援和狀態後，link:bootmedia-shutdown.html["關閉控制器"]。
