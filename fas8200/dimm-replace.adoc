...
summary: 當儲存系統報告 Health Monitor 警示所識別的過多可修正錯誤修正碼 （CECC） 錯誤，或由單一 DIMM 故障所導致的不可修正 ECC 錯誤使儲存系統無法開機 ONTAP 時，您必須更換控制器模組中的 DIMM。 
---
...
[role="lead"]
當儲存系統報告 Health Monitor 警示所識別的過多可修正錯誤修正碼 （CECC） 錯誤，或由單一 DIMM 故障所導致的不可修正 ECC 錯誤使儲存系統無法開機 ONTAP 時，您必須更換控制器模組中的 DIMM。
