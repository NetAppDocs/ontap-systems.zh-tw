...
summary: 若要將 FAS8200 系統還原至完整運作狀態，請還原 NetApp Storage Encryption 組態（如有必要）、安裝新控制器的授權，並依照套件隨附的 RMA 說明，將故障零件退回 NetApp。 
---
= 完成 FAS8200 系統的控制器更換
:allow-uri-read: 
:icons: font
...
[role="lead"]
若要將 FAS8200 系統還原至完整運作狀態，請還原 NetApp Storage Encryption 組態（如有必要）、安裝新控制器的授權，並依照套件隨附的 RMA 說明，將故障零件退回 NetApp。
