...
您可能需要先清除快取模組的內容、然後再進行更換。。雖然快取模組上的資料已加密、但您可能想要清除受損快取模組中的任何資料、並確認快取模組沒有資料：清除快取模組上的資料：「System Controller flash-cache secfire-erase run」（系統控制器快閃快取安全清除執行）。驗證資料是否已從快取模組中清除：「System Controller flash-cache secure-erase show -node_name」（系統控制器快閃快取安全清除show -node_name）
...
+
image::../media/drw_rxl_fcache.png[rw rxl fcache]
