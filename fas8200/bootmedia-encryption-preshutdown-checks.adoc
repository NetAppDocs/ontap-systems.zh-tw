...
summary: 在關閉FAS8200系統上故障的控制器之前，請先驗證加密金鑰支援和狀態。FAS8200系統僅支援手動啟動媒體復原程序。不支援自動啟動媒體恢復。 
---
= 檢查 FAS8200 系統中開機媒體的加密支援情況
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在關閉FAS8200系統上故障的控制器之前，請先驗證加密金鑰支援和狀態。此過程包括檢查ONTAP版本與NetApp卷加密 (NVE) 的兼容性、驗證金鑰管理器配置以及備份加密訊息，以確保在啟動媒體恢復期間的資料安全。
