...
summary: 您需要更換控制器模組中的即時時鐘電池，以確保依賴精確時間同步的服務和應用程式能夠繼續運作。 
---
= 更換 FAS8200 系統中的即時時鐘電池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* 系統中的所有其他元件必須正常運作；如果它們無法正常運作，您必須聯絡技術支援。
...
若要更換RTC電池、請在控制器內找到它們、然後依照特定的步驟順序進行。

. 如果您尚未接地、請正確接地。
. 找到RTC電池。
+
image::../media/drw_rxl_rtc_battery.png[取出並更換 RTC 電池]
...
== 步驟 4：更換 RTC 電池後，重新安裝控制器模組並重設時間和日期
