...
summary: 在FAS8200系統的替換啟動媒體上恢復加密配置。FAS8200系統僅支援手動啟動媒體復原程序。不支援自動啟動媒體恢復。 
---
= 在 FAS8200 系統中恢復開機媒體上的加密
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在FAS8200系統的替換啟動媒體上恢復加密配置。此流程包括完成已啟用板載金鑰管理器 (OKM)、 NetApp儲存加密 (NSE) 或NetApp磁碟區加密 (NVE) 的系統更換後的步驟，以確保安全的資料存取和系統的正常運作。
