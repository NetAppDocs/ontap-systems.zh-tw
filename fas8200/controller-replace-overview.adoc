...
---
= 在更換 FAS8200 系統中的控制器之前，請先查看相關要求。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
如果您應該使用此程序、請注意、四或八個控制器MetroCluster 的控制器的控制器更新程序與HA配對相同。不需要使用MetroCluster特定步驟、因為故障僅限於HA配對、而儲存容錯移轉命令可在更換期間提供不中斷營運的操作。
...
* 更換控制器模組必須支援從舊控制器模組移至新控制器模組或從現有客戶站台庫存新增的任何PCIe卡。
+
