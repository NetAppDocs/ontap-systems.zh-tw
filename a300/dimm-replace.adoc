...
summary: 當系統偵測到越來越多的可修正錯誤修正代碼或不可修正的 ECC 錯誤時，請更換 AFF A300 控制器模組中的 DIMM 記憶體條。未更換故障的 DIMM 記憶體條可能會導致系統崩潰。 
---
= 更換 AFF A300 系統中的 DIMM 記憶體條
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當儲存系統遇到過多的可修正 ECC 錯誤或不可修正 ECC 錯誤時，請更換 AFF A300 系統中的 DIMM，這通常是由單一 DIMM 故障引起的。
