...
summary: '在雙節點MetroCluster配置中完成 FRU 更換後，您可以執行MetroCluster切換操作。這樣就將設定恢復到正常運作狀態，先前受損網站上的同步來源儲存虛擬機器 (SVM) 現在處於活動狀態，並從本機磁碟池提供資料。AFF A300系統僅支援手動啟動媒體復原程式。' 
---
= 以雙節點MetroCluster 的不全功能組態切換回Aggregate AFF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
完成啟動介質更換後，執行MetroCluster切換操作。AFF A300系統僅支援手動啟動媒體復原程式。不支援自動啟動媒體恢復。
