...
summary: 您可以根據儲存系統硬體配置的不同，使用不同的程式來關閉或接管故障控制器。AFF A300系統僅支援手動啟動媒體復原程式。 
---
= 在手動開機恢復過程中，關閉 AFF A300 系統中發生故障的控制器。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
使用適合您配置的相應程式關閉或接管故障控制器。AFF A300系統僅支援手動啟動媒體復原程式。不支援自動啟動媒體恢復。
