...
summary: 將儲存資源的控制權傳回給取代控制器，以便您的AFF C30 或AFF C60 儲存系統可以恢復正常運作。復原過程會根據系統使用的加密類型（無加密、板載金鑰管理員 OKM 加密或外部金鑰管理員 EKM 加密）而有所不同。 
---
= 請將控制器放回您的 AFF C30 或 AFF C60 儲存系統中，以恢復正常運作。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
將儲存資源的控制權傳回給取代控制器，以便您的AFF C30 或AFF C60 儲存系統可以恢復正常運作。復原過程會根據系統使用的加密類型而有所不同：無加密、板載金鑰管理員 (OKM) 加密或外部金鑰管理員 (EKM) 加密。
