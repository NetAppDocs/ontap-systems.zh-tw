...
summary: 將儲存資源的控制權交給更換控制器，讓 AFF C30 或 AFF C60 系統恢復正常運作。 
---
= 可回收並退回控制器 - AFF C30 和 AFF C60
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
將儲存資源的控制權交給更換控制器，讓 AFF C30 或 AFF C60 系統恢復正常運作。回饋程序會因系統使用的加密類型而異：無加密，內建金鑰管理程式（ OKM ）加密或外部金鑰管理程式（ EKM ）加密。
