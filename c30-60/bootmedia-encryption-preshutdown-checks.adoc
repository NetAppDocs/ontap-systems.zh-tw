...
summary: 為確保AFF C30 或AFF C60 儲存系統上的資料安全，您需要驗證啟動媒體上的加密金鑰支援情況及其狀態。請檢查您的ONTAP版本是否支援NetApp磁碟區加密，並在關閉控制器之前檢查金鑰管理員是否處於活動狀態。 
---
= 檢查 AFF C30 或 AFF C60 儲存系統中開機媒體的加密支援
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
為確保 AFF C30 或 AFF C60 儲存系統的資料安全，您需要驗證啟動媒體上的加密金鑰支援情況和狀態。檢查您的 ONTAP 版本是否支援 NetApp 磁碟區加密 （NVE）。在關閉控制器之前，請檢查金鑰管理程式是否處於作用中狀態。
...
在關閉具有損壞開機媒體的控制器之前，請驗證金鑰管理程式組態並備份必要資訊。
