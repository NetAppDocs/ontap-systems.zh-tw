...
summary: 更換 AFF C30 或 AFF C60 儲存系統中的即時時鐘電池（通常稱為鈕扣電池），以確保依賴精確時間同步的服務和應用程式能夠繼續運作。 
---
= 更換 AFF C30 或 AFF C60 儲存系統中的即時時鐘電池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
更換AFF C30 或AFF C60 儲存系統中的即時時鐘 (RTC) 電池（通常稱為連結電池），以確保依賴準確時間同步的服務和應用程式保持運作。

.開始之前
儲存系統中的所有其他元件必須正常運作。如果它們發生故障，請在繼續執行此步驟之前聯絡 https://mysupport.netapp.com/site/global/dashboard["NetApp支援"]。
...
+
您可以透過輸入 `system location-led _off_`命令來關閉它們。如果您不確定 LED 燈是亮著還是熄滅，請透過輸入 `system location-led show`命令來檢查它們的狀態。
...
NOTE: 更換 RTC 電池、插入控制器並開機後，在第一次 BIOS 重置期間，您可能會看到以下錯誤訊息：
