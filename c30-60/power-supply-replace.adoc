...
summary: 當您的 AFF C30 或 AFF C60 儲存系統中的交流電或直流電源供應故障時，請更換電源供應，以確保系統持續獲得穩定運作所需的電力。更換過程包括：斷開故障電源供應與電源的連接、拔除電源線、更換故障電源供應，然後重新連接至電源。 
---
= 在 AFF C30 或 AFF C60 儲存系統中熱插拔電源供應單元
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當您的 AFF C30 或 AFF C60 儲存系統中的交流或直流電源供應器 （PSU） 發生故障或損壞時，請更換此電源供應器，以確保系統持續獲得穩定運作所需的電力。更換過程包括：斷開故障電源供應器與電源的連接、拔除電源線、更換故障電源供應器，然後重新連接至電源。
