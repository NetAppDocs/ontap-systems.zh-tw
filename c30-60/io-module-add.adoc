...
summary: 為您的 AFF C30 或 AFF C60 儲存系統新增 I/O 模組，以增強網路連線能力並擴展系統處理資料流量的能力。 
---
= 在 AFF C30 或 AFF C60 系統新增 I/O 模組
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
為您的 AFF C30 或 AFF C60 儲存系統新增 I/O 模組，以增強網路連線能力並擴充系統的資料流量處理能力。您可以使用此步驟將 I/O 模組新增至正在執行的控制器（作為計劃升級），或新增至維護期間 I/O 模組故障的控制器。

當有可用的插槽或所有插槽都已滿時，您可以將 I/O 模組新增至 AFF C30 和 AFF C60 儲存系統。

.關於這項工作
無論 HA 配對中任一控制器的 I/O 模組目前是否正常運作或發生故障，您都可以在該控制器上執行此程序。由於無法在正在運作的控制器上新增 I/O 模組，因此必須先關閉目標控制器，才能安裝模組。在此程序期間，HA 合作夥伴將接管儲存設備作業，以確保用戶端的資料持續可用。

* 如果目標控制器是*運作中的*（計劃新增的），請按照所寫的步驟進行關機，在該程序中將目標控制器視為具有受損的 I/O 模組。
* 如果目標控制器的 I/O 模組發生故障，請依照指示執行關機步驟。
...
接管並關閉目標控制器，以便其 HA 合作夥伴在您新增 I/O 模組期間繼續提供資料服務。相關步驟指的是關閉承載故障 I/O 模組的控制器。無論您是接管控制器以進行計劃中的新增操作，還是關閉已承載故障 I/O 模組的控制器，這些步驟均適用。
...
.開始之前
* 檢查 https://hwu.netapp.com["NetApp Hardware Universe"^]以確保新的 I/O 模組與您的儲存系統和您正在執行的 ONTAP 版本相容。
