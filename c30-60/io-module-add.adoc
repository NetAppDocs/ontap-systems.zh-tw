...
---
= 為您的 AFF C30 或 AFF C60 儲存系統新增 I/O 模組，以增強網路連線。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
為您的 AFF C30 或 AFF C60 儲存系統新增 I/O 模組，以增強網路連線並擴充系統的資料流量處理能力。您可以使用此程序將 I/O 模組新增至正在執行的控制器（作為計劃升級）或新增至維護期間發生故障的控制器。

當有可用的插槽或所有插槽都已滿時，您可以將 I/O 模組新增至 AFF C30 和 AFF C60 儲存系統。

.關於這項工作
無論 HA 對中的控制器目前是否正常運作，您都可以對任一控制器執行此程序。由於無法在正在運作的控制器上新增 I/O 模組，因此必須先關閉目標控制器才能安裝模組。在程序期間，HA 合作夥伴會接管儲存作業，以便讓用戶端仍可使用資料。

* 如果目標控制器是 *operational* （計畫新增），請依照所寫的關機步驟進行，並將目標控制器視為該內容中的「受損」控制器。
* 如果目標控制器 *受損*，請依照指示執行關機步驟。
...
接手並關閉目標控制器模組，以便其 HA 合作夥伴在您新增 I/O 模組時繼續提供資料。連結的步驟將要關閉的控制器稱為「受損」控制器；無論您是為了計劃性新增而接管運作中的控制器，還是關閉已經受損的控制器，這些步驟都適用。
