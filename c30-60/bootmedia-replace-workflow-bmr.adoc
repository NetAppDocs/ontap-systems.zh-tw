...
summary: 啟動映像的自動恢復涉及系統自動識別並選擇適當的啟動選單選項。它使用合作夥伴節點上的啟動映像在AFF C30 或AFF C60 儲存系統中的替換啟動媒體上重新安裝ONTAP 。 
---
= 使用自動化開機恢復流程的 AFF C30 和 AFF C60 開機媒體更換工作流程
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
啟動映像的自動恢復涉及系統自動識別並選擇適當的啟動選單選項。它使用合作夥伴節點上的啟動映像在AFF C30 或AFF C60 儲存系統中的替換啟動媒體上重新安裝ONTAP 。
