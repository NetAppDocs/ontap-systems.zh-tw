...
summary: 當系統登錄越來越多的可修正錯誤修正碼（ ECC ）時，請更換控制器模組中的 DIMM ；否則會造成系統故障。 
---
= 更換 DIMM - AFF C30 和 AFF C60
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如果偵測到過度的可修正或不可修正的記憶體錯誤，請更換 AFF C30 或 AFF C60 系統中的 DIMM 。此類錯誤可能會導致儲存系統無法開機 ONTAP 。更換程序包括關閉受損的控制器，將其移除，更換 DIMM ，重新安裝控制器，然後將故障零件退回 NetApp 。
