...
summary: 如果偵測到過多可修正或無法修正的記憶體錯誤，請更換AFF C30 或AFF C60 儲存系統中的 DIMM。此類錯誤可能會阻止儲存系統啟動ONTAP。更換過程包括關閉受損的控制器、將其移除、更換 DIMM、重新安裝控制器，然後將故障部件傳回NetApp。 
---
= 更換 AFF C30 或 AFF C60 儲存系統中的 DIMM 記憶體條以修復記憶體容量錯誤
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如果偵測到過多可修正或無法修正的記憶體錯誤，請更換AFF C30 或AFF C60 儲存系統中的 DIMM。此類錯誤可能會阻止儲存系統啟動ONTAP。更換過程包括關閉受損的控制器、將其移除、更換 DIMM、重新安裝控制器，然後將故障部件傳回NetApp。

.開始之前
* 儲存系統中的所有其他元件必須正常運作。如果出現任何問題，請在繼續操作前聯絡 https://mysupport.netapp.com/site/global/dashboard["NetApp支援"]。
