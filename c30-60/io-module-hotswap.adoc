...
summary: 如果 AFF C30 或 AFF C60 儲存系統中的某個乙太網路 I/O 模組發生故障，且您的儲存系統符合所有 ONTAP 版本要求，則可以熱插拔該模組。 
---
= 在 AFF C30 或 AFF C60 儲存系統中熱插拔 I/O 模組
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如果 AFF C30 或 AFF C60 儲存系統中的某個乙太網路 I/O 模組發生故障，且您的儲存系統符合所有 ONTAP 版本要求，則可以熱插拔該模組。
...
+
** _含有受損 I/O 模組的控制器_就是您正在進行 I/O 模組熱插拔的控制器。
** _健康的控制器_是具有受損 I/O 模組的控制器的 HA 合作夥伴。
