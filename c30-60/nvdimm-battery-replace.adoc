...
summary: 當電池開始充電或故障時，請更換 AFF C30 或 AFF C60 系統中的 NV 電池，因為它負責在停電期間保留重要的系統資料。更換程序包括關閉受損的控制器，移除控制器模組，更換 NV 電池，重新安裝控制器模組，以及將故障零件退回 NetApp 。 
---
= 更換 NV 電池 - AFF C30 和 AFF C60
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
當電池開始充電或故障時，請更換 AFF C30 或 AFF C60 系統中的 NV 電池，因為它負責在停電期間保留重要的系統資料。更換程序包括關閉受損的控制器，移除控制器模組，更換 NV 電池，重新安裝控制器模組，以及將故障零件退回 NetApp 。
