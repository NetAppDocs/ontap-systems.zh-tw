...
summary: 在 AFF C30 或 AFF C60 系統中的替換開機媒體上還原加密，以確保資料持續受到保護。更換程序包括驗證金鑰可用度，重新套用加密設定，以及確認資料的安全存取。 
---
= 在開機媒體上還原加密： AFF C30 和 AFF C60
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 AFF C30 或 AFF C60 系統中的替換開機媒體上還原加密，以確保資料持續受到保護。更換程序包括驗證金鑰可用度，重新套用加密設定，以及確認資料的安全存取。
