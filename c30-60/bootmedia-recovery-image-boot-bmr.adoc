...
summary: 在 AFF C30 或 AFF C60 儲存系統中安裝新的開機媒體裝置後，您可以啟動自動開機媒體恢復程序，以從合作夥伴節點還原組態。在恢復程序期間，系統會檢查是否已啟用加密，並確認目前使用的金鑰加密類型。如果已啟用金鑰加密，系統將引導您完成對應的還原步驟。 
---
= 使用自動開機恢復流程，在您的 AFF C30 或 AFF C60 儲存系統上啟動恢復映像。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 AFF C30 或 AFF C60 儲存系統中安裝新的開機媒體裝置後，您可以啟動自動開機媒體恢復程序，以從合作夥伴節點還原組態。在恢復程序期間，系統會檢查是否已啟用加密，並確認目前使用的金鑰加密類型。如果已啟用金鑰加密，系統將引導您完成對應的還原步驟。
