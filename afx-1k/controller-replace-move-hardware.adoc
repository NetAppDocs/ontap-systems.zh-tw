...
summary: 當硬體故障需要更換 AFX 1K 儲存系統中的控制器。更換過程包括移除受損的控制器、將組件移至更換的控制器、安裝更換的控制器以及重新啟動它。 
---
= 更換控制器 - AFX 1K
:allow-uri-read: 
:icons: font
...
[role="lead"]
當硬體故障需要更換 AFX 1K 儲存系統中的控制器。更換過程包括移除受損的控制器、將組件移至更換的控制器、安裝更換的控制器以及重新啟動它。
...
.接下來呢？
更換損壞的 AFX 1K 控制器後，link:controller-replace-system-config-restore-and-verify.html["恢復系統設定"] 。
