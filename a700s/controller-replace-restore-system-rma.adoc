...
summary: 若要恢復 AFF A700s 系統的完整運行，如有必要，請恢復 NetApp Storage Encryption 組態、安裝新控制器的授權、並將故障零件退回 NetApp。 
---
= 完成 AFF A700s 系統的控制器更換工作
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
完成 AFF A700s 系統的控制器更換，方法是還原 NetApp 儲存加密組態（如有必要），安裝新控制器的授權，並將故障零件退回 NetApp。
