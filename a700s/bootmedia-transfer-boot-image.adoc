...
summary: 使用輔助啟動媒體或 USB 隨身碟，將啟動映像傳輸到AFF A700s系統上的替換啟動媒體。AFF A700s系統僅支援手動啟動媒體復原程式。不支援自動啟動媒體恢復。 
---
= 使用 USB 快閃磁碟機將開機映像傳輸到 AFF A700s 系統的開機媒體
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
使用輔助開機媒體（主要方法）或 USB 隨身碟（如果輔助開機媒體還原失敗或  `image.tgz` 檔案遺失），將開機映像傳輸到 AFF A700s 系統上的替換開機媒體。
...
== 選項 1：使用第二個開機媒體的備份還原功能傳輸系統映像

您可以使用控制器模組中第二個開機媒體上的映像，將系統映像安裝到替換的開機媒體。這是控制器模組中具有兩個開機媒體的系統的主要方法。

次要開機媒體上的映像必須包含  `image.tgz` 檔案、且不得報告故障。如果  `image.tgz` 檔案遺失或開機媒體報告故障、您就無法使用此程序。請改用 <<方案二：使用USB傳輸啟動鏡像>>。
...
僅當次要開機媒體還原失敗或在次要開機媒體上找不到  `image.tgz` 檔案時，才應使用此程序。

* 您必須擁有格式化為FAT32的USB隨身碟、且容量至少為4GB。
* 一份與故障控制器運作的 ONTAP 版本相同的鏡像檔。您可以從 NetApp 支援網站的「下載」部分下載對應的鏡像檔。
+
