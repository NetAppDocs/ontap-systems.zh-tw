...
summary: 在雙節點 MetroCluster 組態中完成 FRU 更換後、您可以執行 MetroCluster 切換回復作業。這會將組態恢復至正常運作狀態、先前受損站台上的同步來源儲存虛擬機器（SVM）現在處於作用中狀態、並從本機磁碟集區提供資料。FAS9000 系統僅支援手動開機媒體還原程序。 
---
= 在適用於 FAS9000 系統的雙節點 MetroCluster 組態中切回彙總空間
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
完成開機媒體更換後，請在 FAS9000 系統上執行 MetroCluster 切換回復操作。此程序會將組態恢復到正常運作狀態，先前受損站台上的同步來源儲存設備虛擬機器 （SVM） 現在將作用中，並從本機磁碟集區提供資料。FAS9000 系統僅支援手動開機媒體恢復程序。
