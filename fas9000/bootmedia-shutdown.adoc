...
summary: 您可以根據儲存系統硬體配置的不同，使用不同的程式來關閉或接管故障控制器。FAS9000系統僅支援手動啟動媒體復原程序。 
---
= 在手動開機恢復過程中，關閉 FAS9000 系統中發生故障的控制器。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
使用適合您配置的相應程式關閉或接管故障控制器。FAS9000系統僅支援手動啟動媒體復原程序。不支援自動啟動媒體恢復。
...
完成 NVE 或 NSE 任務後，關閉受損節點。注意：如果您的系統採用雙節點 MetroCluster 組態，請勿使用此步驟。
