...
[role="lead"]
這些詳細的逐步說明將引導您完成 FAS9000 系統的安裝，從機架安裝、佈線到初始設定。如果您需要比快速步驟更深入的安裝指導，請使用這些步驟。
...
您可以根據新系統是 AFF 系統還是 FAS 系統，將其連接到 DS212C、DS224C 或 NS224 機櫃。
...
您必須將每個控制器透過纜線連接至執行 ONTAP 9.8 或更新版本的 AFF A700 或 ASA AFF A700 系統上 NS224 磁碟機機櫃的 NSM 模組。

* 此工作僅適用於執行 ONTAP 9.8 或更新版本的 ASA AFF A700 。
* 每個控制器必須在插槽 3 或 7 中至少安裝一個 X91148A 模組。動畫或插圖顯示該模組已安裝在插槽 3 和 7 中。
...
您必須將每個控制器連接至執行 ONTAP 9.8 或更新版本的 AFF A700 或 ASA AFF A700 系統上 NS224 磁碟機櫃的 NSM 模組。
...
+
如果您的系統有 NS224 磁碟機機櫃，則這些機櫃預設為機櫃 ID 00 和 01。如果您想要變更機櫃 ID，則必須建立一個工具，將其插入按鈕所在的孔中。
