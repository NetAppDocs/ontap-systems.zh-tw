...
summary: 環境變數檢查完畢後，您必須完成已啟用板載金鑰管理員（OKM）、NetApp 儲存加密（NSE）或 NetApp 磁碟區加密（NVE）的系統的特定步驟。FAS9000 系統僅支援手動開機媒體還原程序。 
---
= 在 FAS9000 系統中恢復開機媒體上的加密
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
若要還原 FAS9000 系統中替換開機媒體上的加密，請依照適用於您的金鑰管理程式類型（例如 Onboard Key Manager （OKM））的步驟進行操作。FAS9000 系統僅支援手動開機媒體恢復程序。
