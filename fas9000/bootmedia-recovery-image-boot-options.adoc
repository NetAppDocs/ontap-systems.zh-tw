...
summary: 從恢復映像啟動受損節點的步驟取決於系統是否處於雙節點MetroCluster配置中。FAS9000系統僅支援手動啟動媒體復原程序。 
---
= 在 FAS9000 系統上開機恢復映像
:allow-uri-read: 
:icons: font
...
[role="lead"]
在 FAS9000 系統中安裝替換開機媒體後，從 USB 磁碟機開機恢復映像。啟動受損控制器的步驟取決於系統是否處於雙節點 MetroCluster 組態。FAS9000 系統僅支援手動開機媒體恢復程序。
