...
summary: 在關閉故障控制器並檢查就職加密金鑰狀態之前，必須先檢查故障控制器的狀態，停用自動歸還，並檢查系統運作的 ONTAP 版本。FAS9000 系統僅支援手冊開機媒體恢復程序。 
---
= 檢查 FAS9000 系統中開機媒體的加密支援情況
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
為確保儲存系統的資料安全，請驗證開機媒體上的加密金鑰支援情況和狀態。檢查您的 ONTAP 版本是否支援 NetApp 磁碟區加密 （NVE），並在關閉控制器之前檢查金鑰管理程式是否處於作用中。FAS9000 系統僅支援手動開機媒體恢復程序。
