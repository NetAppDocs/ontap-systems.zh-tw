...
summary: 若要接觸控制器內部的組件，必須先將控制器模組從系統中取出，然後再取下控制器模組上的蓋子。FAS9000系統僅支援手動啟動媒體復原程序。 
---
= 更換開機媒體： FAS9000
:allow-uri-read: 
:icons: font
...
[role="lead"]
若要更換啟動介質，必須拆下損壞的控制器模組，安裝新的啟動介質，並將啟動映像傳輸到 USB 隨身碟。FAS9000系統僅支援手動啟動媒體復原程序。不支援自動啟動媒體恢復。
